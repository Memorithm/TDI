...
use tdi_ai::experimental::tdi21_relational_matched_component::evaluate_matched_component_episode;
...
fn assert_report(
    ...
    assert_eq!(
        report.development_search_work.candidate_rules_evaluated,
        8 * 18
    );
    assert_eq!(
        report.development_search_work.sample_bit_evaluations,
        8 * 18 * 5
    );
    ...
    assert_eq!(
        report.boolean.address_work.anf_term_evaluations,
        (facts + hops) * 6
    );

    assert_eq!(
        report.envelope.attention_history_capacity,
        episode.facts.len()
    );
    assert!(
...
fn task_sized_distractor_episode_has_explicit_component_ceiling_and_scan_cost() {
    ...
    for mode in [AttentionMode::DenseQk, AttentionMode::BinaryQk] {
        let clean_report = evaluate_matched_component_episode(mode, &selected, clean).unwrap();
    ...
    let binary =
        evaluate_matched_component_episode(AttentionMode::BinaryQk, &selected, distracted).unwrap();
    ...
    let dense =
        evaluate_matched_component_episode(AttentionMode::DenseQk, &selected, distracted).unwrap();
