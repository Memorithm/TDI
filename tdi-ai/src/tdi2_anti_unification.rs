...
pub fn anti_unify(
    ...
    let mut next_variable = match maximum_input_variable {
        Some(u32::MAX) => None,
        Some(value) => value.checked_add(1),
        None => Some(0),
    };
    let mut generated_variables = BTreeSet::new();
    anti_unify_incremental(left, right, &mut generated_variables, &mut next_variable)
}
...
pub(super) fn anti_unify_incremental(
    ...
    *next_variable = next_after;
    generated_variables.extend(new_variables);
    Ok(result)
}
...
struct AntiUnificationContext {
    ...
    new_variables: BTreeSet<StructuralVariableId>,
...
impl AntiUnificationContext {
    ...
    fn variable_for_mismatch(
        ...
        self.new_variables.insert(variable);
