...
use super::tdi21_anf_search::{
    AnfSearchError, MAX_SEARCH_CANDIDATES, MAX_SEARCH_DEGREE, MAX_SEARCH_TERMS,
    MAX_SEARCH_VARIABLES, SearchEnvelope,
};
...
use super::tdi21_distributional_objective::{
    ...
    ValidationAdmissionSet, score_validation_policy,
};
...
fn eligible_monomials(envelope: SearchEnvelope) -> Result<Vec<u64>, DistributionalSearchError> {
    ...
    debug_assert!(envelope.variable_count <= MAX_SEARCH_VARIABLES);
    debug_assert!(envelope.max_degree <= MAX_SEARCH_DEGREE);
    debug_assert!(envelope.max_terms <= MAX_SEARCH_TERMS);
    debug_assert!(envelope.max_candidates <= MAX_SEARCH_CANDIDATES);
...
fn candidate_space_size(monomials: usize, max_terms: usize) -> Result<u64, DistributionalSearchError> {
...
///
/// Primary objective is expected Development failure count with every original
/// sample retaining unit weight through its aggregated state's outcome counts.
...
pub fn fit_distributional_anf(
    ...
    let (development_selected_failures, constant, monomials) =
    ...
    let development_score = super::tdi21_distributional_objective::score_development_policy(
        &program,
        development,
    )?;
    debug_assert_eq!(
        development_selected_failures,
        development_score.selected_failures()
    );
    Ok(DistributionalSearchResult {
        envelope,
        program,
        development_selected_failures,
        development_state_oracle_failures: development_score.deterministic_state_oracle_failures(),
        development_conflict_lower_bound: development_score.deterministic_conflict_lower_bound(),
        development_policy_excess_failures: development_score.policy_excess_failures(),
        work,
    })
}
