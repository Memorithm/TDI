//! Lossless architecture-adapter foundation for bounded TDI-8.1 tasks.
//!
//! This module maps one already-generated symbolic [`TaskInstance`] to a
//! deterministic execution schedule shared by A0/A1/A2/A3. It deliberately does
//! not choose recurrent parameters, memory capacities, horizons, population
//! ranges or any TDI-8.2 surface.
//!
//! Two boundaries are explicit:
//! - symbolic `u64` values are encoded losslessly into finite binary64 limbs;
//! - generator-side T3 collision classes remain metadata, while physical A2/A3
//!   projection collisions are measured separately from the concrete table.

use core::{fmt, mem};

use crate::associative_memory::DirectMappedAssociativeMemory;
use crate::full_history_reference::FullHistoryLayout;
use crate::task_generators::{TaskEvent, TaskFamily, TaskInstance, TaskSymbol};
...
/// Number of finite binary64 coordinates required for one exact `u64`.
pub const EXACT_U64_BINARY64_WIDTH: usize = 2;
/// Minimum recurrent-input width for the lossless event frame.
pub const MIN_TASK_EVENT_INPUT_WIDTH: u64 = 9;
/// A0 key width used by this adapter: namespace tag plus two exact `u64` limbs.
pub const A0_TASK_KEY_WIDTH: u64 = 3;
/// A0 value width used by this adapter: two exact `u64` limbs.
pub const A0_TASK_VALUE_WIDTH: u64 = 2;

/// Exact finite binary64 representation of one `u64` as high/low 32-bit limbs.
///
/// Each limb is divided by `2^32`. Because the numerator is at most 32 bits and
/// the denominator is a power of two, every encoded coordinate is exactly
/// representable, finite and in `[0, 1)`.
...
impl ExactU64Binary64 {
    ...
    /// Borrow the two finite binary64 coordinates.
    #[must_use]
...
///
/// The first nine coordinates have fixed lossless semantics. Any additional
/// coordinates are deterministic zero padding. This permits later bounded
/// development to select a concrete input/VSA width without changing task
/// identity or re-encoding already generated symbolic observations.
...
impl TaskAdapterLayout {
    /// Construct an adapter layout large enough for the fixed event frame.
    ...
    /// Exact A0 key/value layout used by the same symbolic schedule.
    #[must_use]
    pub fn a0_layout(self) -> FullHistoryLayout {
        let _ = self;
        FullHistoryLayout::new(A0_TASK_KEY_WIDTH, A0_TASK_VALUE_WIDTH)
            .expect("fixed A0 adapter widths are non-zero")
    }
}

/// One deterministic A0 action derived from a symbolic task event.
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum A0TaskAction {
    /// Add one accessible full-history item.
    Append {
        /// Namespaced exact content key.
    ...
    Read {
        /// Namespaced exact query key.
        key: [f64; A0_TASK_KEY_WIDTH as usize],
        /// Exact expected value for task scoring.
        target: [f64; A0_TASK_VALUE_WIDTH as usize],
    },
}

/// One architecture-neutral event mapped to the concrete A0/A1/A2/A3 APIs.
...
impl TaskEventPlan {
    ...
    /// Lossless event frame consumed identically by A1/A2/A3 recurrent inputs.
    #[must_use]
    pub fn recurrent_input(&self) -> &[f64] {
        &self.recurrent_input
    }

    /// Key passed to the A2/A3 read path for this step.
    #[must_use]
    pub const fn memory_read_key(&self) -> u64 {
        self.memory_read_key
    }

    /// Optional key written by A2/A3 after the recurrent transition.
    #[must_use]
    ...
    /// Full-history action for the contextual A0 control.
    #[must_use]
    pub const fn a0_action(&self) -> A0TaskAction {
        self.a0_action
    }

    /// Exact symbolic target only on task-query events.
    #[must_use]
...
/// Complete deterministic execution schedule for one generated task instance.
#[derive(Clone, Debug, PartialEq)]
...
impl TaskAdapterPlan {
    /// Frozen task family inherited from the symbolic instance.
    #[must_use]
    pub const fn family(&self) -> TaskFamily {
        self.family
    }

    /// Exact generator seed inherited from the symbolic instance.
    #[must_use]
    pub const fn seed(&self) -> u64 {
        self.seed
    }

    /// Caller-supplied adapter layout.
    #[must_use]
    pub const fn layout(&self) -> TaskAdapterLayout {
        self.layout
    }

    /// Dedicated no-write read key selected outside the instance's write-key set.
    #[must_use]
    pub const fn distractor_read_key(&self) -> u64 {
        self.distractor_read_key
    }

    /// Ordered event schedule shared by all arms.
    #[must_use]
...
/// Deterministic projection/collision audit for one A2/A3 table layout/seed.
...
impl ProjectionAudit {
    /// Number of association/payload writes in the schedule.
    #[must_use]
    pub const fn planned_writes(self) -> u64 {
        self.planned_writes
    }

    /// Number of distinct physical direct-mapped addresses occupied after all writes.
    #[must_use]
    pub const fn distinct_occupied_addresses(self) -> u64 {
        self.distinct_occupied_addresses
    }

    /// Number of writes replacing a different key at the same physical address.
    #[must_use]
    pub const fn physical_replacement_collisions(self) -> u64 {
        self.physical_replacement_collisions
    }

    /// Query reads whose exact key is resident at query time.
    #[must_use]
    pub const fn query_hits(self) -> u64 {
        self.query_hits
    }

    /// Query reads whose projected address contains another key.
    #[must_use]
    pub const fn query_collision_misses(self) -> u64 {
        self.query_collision_misses
    }

    /// Query reads whose projected address is empty.
    #[must_use]
    pub const fn query_empty(self) -> u64 {
        self.query_empty
    }

    /// Reuse count of generator-side collision classes. This is metadata, not a
    /// physical collision count.
    #[must_use]
    pub const fn generator_class_reuses(self) -> u64 {
        self.generator_class_reuses
    }

    /// Physical replacement collisions where old/new symbolic associations also
    /// shared the same generator-side collision class.
    #[must_use]
...
pub enum TaskAdapterError {
    /// Recurrent input is too narrow for the lossless event schema.
    InputWidthTooSmall {
    ...
    /// Platform-independent dimension cannot be represented by this host.
    ...
    /// Host vector byte capacity is not representable.
    ...
    /// Host allocator rejected a validated reservation.
    ...
    /// Encoded limb is non-finite, out of range or not on the exact `2^-32` grid.
    NonCanonicalEncodedLimb {
        /// Limb index.
        index: usize,
        /// Original binary64 bits for deterministic diagnostics.
        value_bits: u64,
    },
    /// Adapter event count does not fit `u64` provenance fields.
    EventCountTooLarge,
    /// Could not choose a distractor read key outside the finite write-key set.
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InputWidthTooSmall { minimum, actual } => write!(
                formatter,
                "TDI-8.1 task adapter requires recurrent input width >= {minimum}, got {actual}"
            ),
...
///
/// The audit is a deterministic projection simulation. It does not inspect or
/// mutate payload/recurrent state and therefore cannot manufacture a task
/// success result. Generator class reuse and physical slot collisions are
/// reported separately.
pub fn audit_associative_projection(
    ...
    let mut occupied = reserve_vec("projection occupied addresses", plan.events.len())?;
    let mut seen_classes = reserve_vec("projection generator classes", plan.events.len())?;
...
mod tests {
    ...
    use crate::task_generators::{T1Config, T2Config, T3Config, TaskEvent, generate_t1, generate_t2, generate_t3};

    #[test]
    fn exact_u64_codec_round_trips_edge_values_bit_exactly() {
        for value in [0, 1, u32::MAX as u64, 1u64 << 32, u64::MAX] {
            let encoded = ExactU64Binary64::encode(value).coordinates();
            assert!(encoded.iter().all(|coordinate| coordinate.is_finite()));
            assert!(encoded.iter().all(|coordinate| (0.0..1.0).contains(coordinate)));
            assert_eq!(ExactU64Binary64::decode(encoded).expect("canonical limbs"), value);
        }
    }

    #[test]
    fn exact_u64_decoder_rejects_noncanonical_coordinates() {
        assert!(matches!(
    ...
    #[test]
    fn adapter_layout_has_no_default_and_only_enforces_minimum_lossless_width() {
        assert_eq!(
    ...
    #[test]
    fn one_symbolic_t1_instance_maps_to_one_shared_recurrent_schedule_and_exact_a0_targets() {
        ...
        assert!(plan
            .events()
            .iter()
            .all(|event| event.recurrent_input().len() == MIN_TASK_EVENT_INPUT_WIDTH as usize));
        assert!(plan.events().iter().all(|event| event.recurrent_input().iter().all(|x| x.is_finite())));
        ...
        for event in plan.events() {
            if let A0TaskAction::Read { target, .. } = event.a0_action() {
                ...
                assert_eq!(Some(target_code), event.expected_target().map(|symbol| symbol.code()));
            }
        }
    }

    #[test]
    fn t2_payload_writes_and_queries_use_the_same_derived_memory_keys() {
        ...
        let writes: Vec<_> = plan
            .events()
            .iter()
            .filter_map(|event| event.memory_write_key())
            .collect();
        let queries: Vec<_> = plan
            .events()
            .iter()
            .filter(|event| event.expected_target().is_some())
            .map(|event| event.memory_read_key())
            .collect();
        assert_eq!(writes, queries);
    }

    #[test]
    fn distractor_read_key_is_never_a_logical_write_key() {
        let instance = generate_t1(31, T1Config::new(7, 5, 3).expect("T1 config")).expect("T1");
        let plan = build_task_adapter_plan(
            &instance,
            TaskAdapterLayout::new(MIN_TASK_EVENT_INPUT_WIDTH).expect("layout"),
        )
    ...
    #[test]
    fn projection_audit_separates_generator_classes_from_actual_slot_collisions() {
        let instance = generate_t3(41, T3Config::new(8, 3, 4, 20, 3).expect("T3 config")).expect("T3");
        let plan = build_task_adapter_plan(
            &instance,
            TaskAdapterLayout::new(MIN_TASK_EVENT_INPUT_WIDTH).expect("layout"),
        )
    ...
    #[test]
    fn source_events_are_preserved_exactly_in_order() {
        let instance = generate_t2(99, T2Config::new(2, 2).expect("T2 config")).expect("T2");
        let plan = build_task_adapter_plan(
            &instance,
            TaskAdapterLayout::new(MIN_TASK_EVENT_INPUT_WIDTH).expect("layout"),
        )
        .expect("plan");
        let recovered: Vec<TaskEvent> = plan.events().iter().map(|event| event.source_event()).collect();
