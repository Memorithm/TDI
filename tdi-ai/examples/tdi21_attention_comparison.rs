...
use tdi_ai::experimental::tdi21_stream::{Event, MemoryMode, StepOutput, StreamConfig, StreamError};
...
fn main() {
    ...
    let pair = vec![write(1, 1), write(5, 0), Event::Recall { key: 1 }, Event::Recall { key: 5 }];
    let saturated = vec![
        write(1, 1), write(5, 0), write(9, 1),
        Event::Recall { key: 1 }, Event::Recall { key: 5 }, Event::Recall { key: 9 },
    ];
    let reverse = vec![
        write(1, 1), write(3, 3), write(5, 5), write(9, 9), Event::Recall { key: 3 },
    ];
    let mixed = vec![
        Event::Recall { key: 7 }, write(7, 0), Event::Recall { key: 7 }, write(7, 3),
        Event::Write {
            key: 7, payload: BooleanState::from_bits(255), marker: BooleanState::from_bits(3),
        },
        Event::Recall { key: 7 }, write(0, 5), Event::Conjunction { left: 7, right: 0 },
        Event::Recall { key: 9 }, Event::Conjunction { left: 7, right: 9 }, Event::Recall { key: 0 },
    ];
    println!("{ATTENTION_SEMANTICS};source_sha={sha};scope=development-only;training=none");
    println!("NOT matched total budgets: Boolean substrate ceiling=518 bits; attention history capacity=16 writes");
    for (id, events, expected_other) in [
        ...
        for mode in [AttentionMode::DenseQk, AttentionMode::BinaryQk] {
            let config = AttentionConfig {
                mode, history_capacity: 16, payload_bits: 8, max_events: 8192,
            };
            ...
            println!(
                ...
                mode.arm(), score.accuracy_ratio(), reference.work(), reference.footprint(),
            );
        }
        for (index, mode) in [MemoryMode::Direct, MemoryMode::TwoWay].into_iter().enumerate() {
            let config = StreamConfig {
                mode, slots: fit_slots(mode, 518).unwrap(), payload_bits: 8,
                max_events: 8192, route_salt: 0,
            };
        ...
        for (index, arm) in [ControlArm::NoMemory, ControlArm::ExactDictionary].into_iter().enumerate() {
