...
- **B4** — B3 plus synthesized/searched `F2` / algebraic-normal-form (Zhegalkin) routing; admission-policy machinery is implemented in Stage-0 and relational binding/composition is the current development frontier;
...
The current relational frontier, documented in [TDI-21.0 relational binding](docs/TDI-21.0-RELATIONAL-BINDING.md), tests a broader attention function: subject–relation binding and bounded multi-hop composition without token-pair scoring or history scans. Development and Validation use renamed identifier namespaces with an independent dictionary oracle. A direct symbolic address and an exactly equivalent ANF/Zhegalkin identity encoder must agree; this isolates algebraic address composition before any learned relational encoder is evaluated.
