...
Status: **development-only scaffold on an unlanded branch; not frozen; not confirmatory; no final/holdout material**.
...
The objective does **not** subtract the v1 feature-conflict floor before selection. The raw expected failure count is the operational quantity being optimized. The decomposition is reported separately so interpretation remains honest.

## Error decomposition retained after selection

The selected result reports:
