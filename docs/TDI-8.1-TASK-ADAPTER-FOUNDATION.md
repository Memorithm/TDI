...
The next evaluator layer may consume this schedule to execute A0/A1/A2/A3 and emit typed per-generator observations. Before those observations can support a primary-cell classifier, TDI-8.1 still needs explicit task output/readout semantics for recurrent arms, exact operation accounting, typed provenance/rejection records, and a qualified paired interval method built on the merged resampling foundation.
