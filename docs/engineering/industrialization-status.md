...
**Current exact-head qualification note (2026-09-17):** the current default-branch checkpoint for this refresh is `807bc13d1d6389cb0314cc18be6c9bd7dc31126e` (qualified sensitivity/ablation PR #387, after qualified CI hardening PR #467). For current qualification state of earlier Lot-H/operational/export work and active candidates #388/#390/#464, use [`industrialization-qualification-audit-2026-09-16-r2.md`](industrialization-qualification-audit-2026-09-16-r2.md), which is a reconciliation overlay on the historical [`industrialization-qualification-audit-2026-09-16.md`](industrialization-qualification-audit-2026-09-16.md). Where an older row below conflicts with that evidence, use the newer exact-head evidence and retain the older row only as historical programme context. Merge presence alone is not a qualification result.
...
1. Continue from default branch `26588a3a0d714e6cbf1b9d3301e9001f60b5e83e`. #388 measured engine benchmarks qualified on exact head `47cc7c5cb06e945935b7804c1b2d1e31baef4955` with the applicable GitHub workflows successful and all returned material review threads resolved before squash merge as `26588a3a0d714e6cbf1b9d3301e9001f60b5e83e`. Its stored observations remain Development-scoped evidence, not universal performance claims.
2. #409 research consultation/reporting is refreshed from that default branch at candidate head `48a385c25d9857b2459411f34334e0568d8114f8`. Forge and SciRust consumers now pin the qualified merge revisions `28067ab0aa1d52a2260d9bb2bf35a346547a292a` and `be7fcca3b31cedf722d71a2a56db8f6d088037cf`. Its dedicated reporting workflow has succeeded on that exact head, but other applicable exact-head gates remain queued or in progress; the PR therefore remains draft.
3. #411 FLAT/NNIS attention adapters is merged forward from current `main` at candidate head `bf1107a9b7c07c785defac0adfda1d5e5b2db98e`. The refresh preserves the newer sensitivity/search CLI dispatch and repairs nested Cargo qualification so the isolated, locked integration workspace can resolve its exact FLAT/NNIS git sources without weakening the parent TDI workspace's vendored/offline policy. Local Rust 1.89 default/all-feature build+Clippy and 21 exact-pin integration/partner tests pass; GitHub exact-head qualification is still running, so the PR remains draft and no CUDA/hardware/performance qualification is implied.
4. Treat the old #296 SciRust candidate pin as closed only through qualified corrective PR #466. SciRust #1452 qualified and merged as `be7fcca3b31cedf722d71a2a56db8f6d088037cf`; unrelated Dream/Thor negative hardware/runtime evidence must remain isolated from statistics qualification.
5. Continue hardware-specific FLAT/NNIS qualification only when the required physical evidence is available. Do not convert queued/pending workflows, local passes, merge presence, or unavailable hardware into qualification claims.
