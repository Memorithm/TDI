...
2. Refresh #388 measured engine benchmarks and #464 runner-load scoping from current `main` before interpreting their next matrices. Both are draft candidates and were reported non-mergeable against current `main` at this checkpoint; conflict resolution/synchronization is required before exact-head qualification can begin again.
