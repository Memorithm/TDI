...
**Current exact-head qualification note (2026-09-17):** the current default-branch checkpoint for this refresh is `30c7c6f00a400ab79cff504233fe923d3a9ced64`, the qualified squash merge of research consultation/reporting PR #409 from exact head `48a385c25d9857b2459411f34334e0568d8114f8`. Measured-engine benchmark PR #388 had already qualified and merged as its parent `26588a3a0d714e6cbf1b9d3301e9001f60b5e83e`. Older qualification-audit files remain historical reconciliation overlays; when an older row conflicts with this newer exact-head evidence, retain the older row only as programme history. Merge presence alone is not a qualification result.
...
| Q30 | Final-head CI, merge SHA and post-merge verification | Qualified for the current TDI operational/reporting delivery through #409: all returned workflows on exact head `48a385c25d9857b2459411f34334e0568d8114f8` completed successfully before merge `30c7c6f00a400ab79cff504233fe923d3a9ced64`, and the default branch was re-read at that merge. Every later candidate still requires its own exact-head qualification; this row is not transferable evidence. |
...
1. Continue from default branch `a7c635c4a9bcbde8e458f7fe442d9e0c3260dba8`. Status refresh PR #468 qualified on exact head `aa8a341905eb835474b6e2cf2c5613ef6ef3221b` and merged as `c0ba59c8071483b8cefeae2db21855ab88bb3be0`; TDI-24.0 arithmetic-hardening PR #471 then qualified on exact head `b18f846fcb4e68943d28a780d5b9266d612b2b8b` and merged as the current default-branch commit `a7c635c4a9bcbde8e458f7fe442d9e0c3260dba8`. #471 preserves the antisymmetric chiral pairing success domain and adds the exact large-coordinate regression identified during review. This is software-contract evidence only; it is not a scientific, hardware, or performance result.
2. #411 FLAT/NNIS attention adapters has been merged forward from that default branch at candidate head `4ddcbc382c1e089b43b47a263b1d7838ed56ecf0`. The refresh preserves both the attention/NNIS commands and the newly qualified reporting/viewer surface. Local Rust 1.89 default build/format/Clippy and Python compilation pass with the nested Cargo workspace invoked outside the parent vendored/offline config scope. The PR remains draft until all applicable GitHub workflows succeed on this exact head. Physical CUDA execution/parity, GPU timing, VRAM and model-quality claims remain blocked on real hardware evidence.
3. #384 TDI-23.2 bounded local rewrite calculus remains a draft candidate at head `b1c70b19b51df25a03d4d6924b7101eebe1d2fe1` and is currently behind `main`. Its previous readiness run exposed a CI-environment failure because the self-hosted Rust 1.85 toolchain lacked `rustfmt`; that infrastructure defect is being repaired separately rather than treated as a scientific/code failure. The documented TDI-23.1 freeze prerequisite must still be satisfied before promotion/merge. Rewrite search, recursive optimization, FLAT integration and confirmatory/final execution remain unauthorized.
4. Treat the old #296 SciRust candidate pin as closed only through qualified corrective PR #466. SciRust #1452 qualified and merged as `be7fcca3b31cedf722d71a2a56db8f6d088037cf`; unrelated Dream/Thor negative hardware/runtime evidence remains separate from statistics qualification.
5. Lot H requalification PR #470 has been merged forward again after #471 and is now at candidate head `a1408dd67bc661ee4f5824f4c635dfbd115c9596`. The PR remains candidate-only while its exact-head workflow matrix is queued/in progress; it creates no execution, actuation, scientific-stage, holdout, hardware or performance authority.
6. CI repair PR #473 is a non-scientific infrastructure candidate at exact head `d9d8b6f649faa5493137d5375c09819ea658532f`. It pins exact-head checkout and explicitly provisions Rust 1.85.0 `rustfmt`/`clippy` for `TDI-9.1 readiness integrity`, after #408 and #384 both failed on a runner missing `rustfmt`. The full readiness script passes locally on Rust 1.85.0 while TDI-9.1 remains `unresolved_blocking` with 1/14 fields pinned and no TDI-9.2 execution surface. #473 must not merge until all applicable exact-head gates are green.
7. Continue hardware-specific FLAT/NNIS qualification only when the required physical evidence is available. Do not convert local passes, queued/pending workflows, merge presence, or unavailable hardware into qualification claims.
