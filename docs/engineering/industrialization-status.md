...
- Lot G3 exact execution-admission binding: candidate in PR #272. It pins Hub #54 merge `7187d72e025e0a7a88b52d9fc4675bae26581f76`, requires the full WorkflowSpec to equal Graph/v1 compilation and exact admission-pin coverage to match every Graph component version, manifest digest and capability-contract version. It grants only Hub execution-admission evidence; scientific-stage authorization remains hard-false. Qualification is pending exact-head CI and review.
...
| E Hub bridge | qualified structural preview only | PR #254 `compile_hub_workflow_preview()` remains a non-authoritative structural compiler pinned to its audited source; later G3 consumes the separately qualified Hub exact-admission surface rather than rewriting Lot E. |
...
| G3 exact workflow execution admission | candidate | PR #272 consumes Hub #54 only: exact WorkflowSpec equality plus exact step/component-version/manifest/capability-contract pins. A combined v3 artifact binding requires the same workflow and admitted producing step, keeps authoritative publication evidence, sets Hub execution admission true and keeps scientific-stage authorization false. |
...
| Q11 | candidate for authoritative execution admission | PR #254 remains the qualified Graph/v1 semantic/structural base. Hub #54 now qualifies exact registry admission upstream; TDI PR #272 is the candidate adapter and must pass exact-head qualification before Q11 can be promoted. |
...
| Q18 | candidate | PR #272 binds Hub #54 exact execution admission to TDI Graph/v1 and composes it with G2 publication evidence only on identical workflow/step lineage; exact-head qualification is pending. |
| Q19-Q30 | planned | cross-repository integrations, statistics, CLI/viewer/exports, hardware adapters, docs and final integrated qualification. |
...
- `ExperimentSpec/v1`, CheckpointManifest/v1, Graph/v1 and all G1-G3 Hub evidence bindings are infrastructure contracts and must not be used to synthesize or authorize a protected/final series surface.

## Next

1. Qualify TDI PR #272 on its exact final head and promote G3/Q11/Q18 only after all applicable gates and material P1/P2 review findings are green/resolved.
2. Add ElasticXxx, Forge, SciRust, FLAT-ATTENTION and NNIS integrations only through qualified versioned contracts.
