...
5. **EXACT** identity-ordering control on nondegenerate samples.
6. Candidate (non-frozen) Green-band response observables over TDI-10 `GreenBands`:
   mid-diagonal Green, Green trace, mean-abs off-diagonal Green.
7. Dimension-only ranking key that does not consult response observables.
8. Machine-readable Stage-0 freeze **template** with every scientific field
   `unresolved_blocking` and both execution flags `false`.
...
## Next engineering slice after bootstrap integrity

Declare and review candidate freeze resolutions for operator populations,
ranking metric, tie policy, normalization and split discipline **without**
authorizing confirmatory execution; then implement TDI-12.1 synthetic evaluator
fixtures under those resolutions.
