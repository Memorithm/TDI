...
- non-executing model-observation registry / timing / H11-A eligibility scaffolding
  (candidates only; does not pin `exact_observation_registry`,
  `exact_observation_timing`, or `primary_pre_assertion_eligibility`);
- non-executing Development/Validation population-derivation scaffolding
  (candidate only; does not pin `development_validation_population_derivation`);
...
Post-#218 scout found no model/adapter/tokenizer identifier that can be pinned;
...
- non-executing registry / timing / H11-A eligibility scaffolding (candidate only;
  **not a pin**):
  `docs/TDI-11.2-MODEL-OBSERVATION-REGISTRY.md`,
  `tdi-ai/src/hallucination_model_observation_registry.rs`,
  `scripts/check-tdi11.2-model-observation-registry.sh`
- non-executing Dev/Val population-derivation scaffolding (candidate only;
  **not a pin**):
  `docs/TDI-11.2-MODEL-OBSERVATION-POPULATION.md`,
  `tdi-ai/src/hallucination_model_observation_population.rs`,
  `scripts/check-tdi11.2-model-observation-population.sh`
...
DiagonalOnlyWidthLadder closed forms do not pin registry / timing / eligibility /
population derivation / resource_accounting_contract / typed_rejection_contract /
provenance_contract. Rejection/provenance/accounting/registry/population
