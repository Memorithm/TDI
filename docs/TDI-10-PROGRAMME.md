...
The elementary subunit-product arc **TDI-10.5–10.12** is now closed (pointwise counterexample → uniform geometric → remainder divergence → trichotomy → harmonic rate → equivalence → exp bounds → Cesàro rate). Next frontiers remain slowly-varying Jacobi / soft-edge — do not invent those theorems from this numbering alone.

## Next frontiers (not yet authorized by numbering alone)

- broader operator families that realize a uniform `rho`, a harmonic remainder rate, or a divergent remainder (or fail to) beyond the named witnesses;
