...
The elementary subunit-product arc **TDI-10.5–10.12** is closed (pointwise counterexample → uniform geometric → remainder divergence → trichotomy → harmonic rate → equivalence → exp bounds → Cesàro rate). **TDI-10.13** opens the operator-family chapter by registering those witnesses as named cavity-transport families; it does not invent slowly-varying or soft-edge theorems.

## Next frontiers (not yet authorized by numbering alone)

- further operator / coefficient classes that induce `alpha_k` beyond zero-drift cavity maps and frozen contractions;
