...
- **TDI-10.17** — **EXACT** family ↔ TDI-10.4 affine-unrolling link: zero-drift `F_S`/`F_U`/`F_D` chains collapse to `E_n = A_n E_0`; constant-drift `F_U` geometric `B_n = δ(1-ρ^n)/(1-ρ)` and `F_D` closed `B_n`; **REFUTED** that Type-U product decay alone forces cavity-error → 0 under nonzero constant drift
- **TDI-10.18** — **EXACT** operator-family finite hypothesis checklist: Item D = TDI-10.9 harmonic lower bound `1-alpha_k ≥ L/k` eventually; Item T = TDI-10.2 one-step identity on family-realized steps; **REFUTED** that informal `alpha_k → 1` / “slow variation” alone meets Item D; **REFUTED** that a finite-window harmonic bound alone forces product → 0
- **TDI-10.19** — **EXACT** reverse/cross family composition: `F_D`/`F_S` then `F_U` closed products `1/(m+1)·ρ^n` and `(m+2)/(2(m+1))·ρ^n`; `F_D`↔`F_S` closed products; **REFUTED** that a Type-S prefix blocks Type-D suffix decay; **REFUTED** that a finite Type-D prefix forces composed liminf 0 under Type-S; **REFUTED** that `F_D`/`F_S` block order is immaterial
- **TDI-10.20** — **EXACT** three-block concatenation + interleaved pair-schedules: mixed `{U,D,S}` permutation closed forms; same-family products under index restart; `(UD)^k = (ρ/2)^k`, `(US)^k = (3ρ/4)^k`, `(DS)^k = (3/8)^k`; **REFUTED** that three-block `{U,D,S}` order is immaterial; **REFUTED** that interleaving vs blocking is immaterial; **REFUTED** that index-restart is immaterial for Type-D / Type-S; **REFUTED** that a Type-S middle erases Type-U prefix decay uniformly in `ℓ`; **REFUTED** that a finite Type-D prefix plus Type-S middle forces liminf 0 under a Type-U suffix of fixed `n`

The elementary subunit-product arc **TDI-10.5–10.12** is closed (pointwise counterexample → uniform geometric → remainder divergence → trichotomy → harmonic rate → equivalence → exp bounds → Cesàro rate). **TDI-10.13** opens the operator-family chapter; **TDI-10.14** bridges the frozen Toeplitz contraction to Type-U `ρ`; **TDI-10.15** records exact `F_U`→`F_D`/`F_S` concatenation calculus; **TDI-10.16** records exact κ(a,b) domain/monotonicity and the matched TDI-10.3 factorization → Type-U link; **TDI-10.17** links TDI-10.2/10.4 affine unrolling into named-family closed products and constant-drift `B_n` formulae; **TDI-10.18** packages the TDI-10.9 harmonic comparison as an operator-family finite hypothesis checklist (with TDI-10.2 Item T on family steps); **TDI-10.19** completes the two-block named-family concatenation table with reverse Type-U suffixes and `F_D`↔`F_S` cross products; **TDI-10.20** records exact three-block products, interleaved pair-schedules, and the index-restart distinction for Type-D / Type-S — still without inventing slowly-varying or soft-edge theorems.

## Next frontiers (not yet authorized by numbering alone)

- further operator / coefficient classes that induce `alpha_k` beyond zero-drift cavity maps, frozen contractions, named-family two- and three-block concatenations (including TDI-10.15/10.19/10.20), interleaved pair-schedules, the κ(a,b) domain calculus, the family↔affine-unrolling / constant-drift closed forms, and the finite hypothesis checklist (Item D / Item T);
- four-or-more-block concatenations / richer interleaved family schedules with closed products;
- slowly varying Jacobi hypotheses with explicit remainder windows (see TDI-10.13 FORMAL pointer — not closed by TDI-10.18's finite checklist, TDI-10.19's reverse/cross products, or TDI-10.20's three-block / interleave calculus);
