...
grep -Fq 'authorization_state = "NOT_AUTHORIZED"' docs/TDI-7.2-FINAL-HOLDOUT-SELECTION.toml \
    || fail "final seed-selection decision record is not explicitly unauthorized"
...
if ((${#forbidden[@]} != 0)); then
    ...
    printf '  %s\n' "${forbidden[@]}" >&2
    fail "TDI-7.2 must remain unarmed until population size, seed selection and arming transition are separately reviewed"
fi
