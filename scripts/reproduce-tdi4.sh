...
PREREG_HASH="docs/TDI-4-TARGET-GEOMETRY-PREREGISTRATION.sha256"
EVALUATOR_HASH="docs/TDI-4-TARGET-GEOMETRY-EVALUATOR.sha256"
SCIENTIFIC_HASH="docs/TDI-4-SCIENTIFIC-CODE.sha256"
RESULT_DIR="results"
RESULT_FILE="${RESULT_DIR}/tdi-target-geometry.log"

export CARGO_TARGET_DIR="${CARGO_TARGET_DIR:-target}"

echo "=== REPRODUCTION TDI-4 ==="
...
if [ -e "$RESULT_FILE" ]; then
    echo "ERREUR : un résultat TDI-4 existe déjà"
    ls -lh "$RESULT_FILE"
    exit 1
fi
...
"$CARGO_TARGET_DIR/release/tdi-target-geometry" |
    tee "$RESULT_FILE"
