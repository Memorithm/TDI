...
PREREG_HASH="docs/TDI-5.2-INDEPENDENT-OVERLAP-ABLATION-PREREGISTRATION.sha256"
EVALUATOR_HASH="docs/TDI-5.2-INDEPENDENT-OVERLAP-ABLATION-EVALUATOR.sha256"
SCIENTIFIC_HASH="docs/TDI-5.2-SCIENTIFIC-CODE.sha256"
...
RESULT_DIR="results/tdi5.2-independent-overlap-ablation"
RESULT_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v52.log"
METADATA_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v52.metadata.txt"
RESULT_HASH_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v52.log.sha256"
COMPLETION_MARKER="${RESULT_DIR}/tdi-independent-overlap-ablation-v52.complete"
LOCK_DIR="${RESULT_DIR}/.tdi5.2.lock"

BINARY_NAME="tdi-independent-overlap-ablation-v52"
...
require_clean_git() {
    if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
        log_error "repository must be clean before the full TDI-5.2 evaluation"
        git status --short
        exit 1
    fi
}
...
refuse_existing_output() {
    if [[ -e "$COMPLETION_MARKER" ]]; then
        log_error "a completed TDI-5.2 run already exists"
    ...
    if [[ -e "$RESULT_FILE" || -e "$RESULT_HASH_FILE" || -e "$METADATA_FILE" ]]; then
        log_error "incomplete TDI-5.2 output exists; refusing to overwrite it"
        ls -lh -- "$RESULT_DIR" >&2
        exit 1
    fi
}
...
acquire_lock() {
    ...
    if ! mkdir -- "$LOCK_DIR" 2>/dev/null; then
        log_error "another TDI-5.2 reproduction appears to be running: ${LOCK_DIR}"
        exit 1
    fi
...
write_initial_metadata() {
    ...
    {
        printf 'experiment=TDI-5.2 independent overlap ablation\n'
        printf 'start_utc=%s\n' "$start_timestamp"
        printf 'commit=%s\n' "$(git rev-parse HEAD)"
        printf 'repository=%s\n' "$ROOT"
...
verify_complete_output() {
    ...
    # Section 17 requires the final TDI-5.2A through TDI-5.2E verdicts and
    # the TDI-5.2C classification; Section 19 item 8 requires verifying
    # all final criterion lines, not just one, so every verdict below
    # must be present.
...
log_info "verifying TDI-5.2 hashes"
...
set +e
"$BINARY_PATH" 2>&1 | tee "$RESULT_FILE"
RUN_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$RUN_STATUS" -ne 0 ]]; then
    log_error "TDI-5.2 evaluator failed with status ${RUN_STATUS}"
    exit "$RUN_STATUS"
fi
...
log_info "TDI-5.2 reproduction completed"
