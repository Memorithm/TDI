#!/usr/bin/env bash
#
# TDI-6.1 reproduction script — the literal spectral gap and mixing time (does
# the GKT-vs-GK overlap signal survive the *actual* second eigenvalue and the
# ε-mixing time of the one-step Noop kernel?). This is the FIRST TDI-6
# experiment and the first to relax bit-exactness — only for the two spectral
# descriptors. This script performs the real, preregistered 120,000-record
# TDI-6.1 run (3 blocks × 40,000 accepted records) exactly once.
#
# The real command, reserved for a deliberate human action, is:
#
#   TDI61_CONFIRM_FULL_RUN=I_ACCEPT_THE_TDI61_FREEZE_RULE \
#     bash scripts/reproduce-tdi6.1.sh
#
...
#
# Reproduction is tolerance-based, not byte-exact (Section 12): on the
...
# metrics may differ in the last f64 digits, but the printed 6.1A/6.1B/6.1C
...
TDI61_CONFIRM_VAR="TDI61_CONFIRM_FULL_RUN"
TDI61_CONFIRM_VALUE="I_ACCEPT_THE_TDI61_FREEZE_RULE"

PREREG_HASH="docs/TDI-6.1-SPECTRAL-GAP-MIXING-TIME-PREREGISTRATION.sha256"
EVALUATOR_HASH="docs/TDI-6.1-SPECTRAL-GAP-MIXING-TIME-EVALUATOR.sha256"
SCIENTIFIC_HASH="docs/TDI-6.1-SCIENTIFIC-CODE.sha256"
...
RESULT_DIR="results/tdi6.1-spectral-gap-mixing-time"
RESULT_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v61.log"
METADATA_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v61.metadata.txt"
RESULT_HASH_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v61.log.sha256"
COMPLETION_MARKER="${RESULT_DIR}/tdi-independent-overlap-ablation-v61.complete"
LOCK_DIR="${RESULT_DIR}/.tdi6.1.lock"

BINARY_NAME="tdi-independent-overlap-ablation-v61"
...
require_full_run_confirmation() {
    local -r actual="${!TDI61_CONFIRM_VAR:-}"

    if [[ "$actual" != "$TDI61_CONFIRM_VALUE" ]]; then
        log_error "refusing: ${TDI61_CONFIRM_VAR} must be set to the exact value ${TDI61_CONFIRM_VALUE}"
        log_error "the real command is:"
        log_error "  ${TDI61_CONFIRM_VAR}=${TDI61_CONFIRM_VALUE} bash ${BASH_SOURCE[0]}"
        exit 1
    fi
}
...
require_clean_git() {
    if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
        log_error "repository must be clean before the full TDI-6.1 evaluation"
        git status --short
        exit 1
    fi
}
...
refuse_existing_output() {
    if [[ -e "$COMPLETION_MARKER" ]]; then
        log_error "a completed TDI-6.1 run already exists"
    ...
    if [[ -e "$RESULT_FILE" || -e "$RESULT_HASH_FILE" || -e "$METADATA_FILE" ]]; then
        log_error "incomplete TDI-6.1 output exists; refusing to overwrite it"
        ls -lh -- "$RESULT_DIR" >&2
        exit 1
    fi
}
...
acquire_lock() {
    ...
    if ! mkdir -- "$LOCK_DIR" 2>/dev/null; then
        log_error "another TDI-6.1 reproduction appears to be running: ${LOCK_DIR}"
        exit 1
    fi
...
write_initial_metadata() {
    ...
    {
        printf 'experiment=TDI-6.1 literal spectral gap and mixing time\n'
        printf 'start_utc=%s\n' "$start_timestamp"
        printf 'commit=%s\n' "$(git rev-parse HEAD)"
        printf 'repository=%s\n' "$ROOT"
...
verify_complete_output() {
    ...
    # TDI-6.1 preregistration Sections 19-20 require the final 6.1A/6.1B/6.1C
...
log_info "verifying TDI-6.1 hashes"
...
if [[ "$RUN_STATUS" -ne 0 ]]; then
    log_error "TDI-6.1 evaluator failed with status ${RUN_STATUS}"
    exit "$RUN_STATUS"
fi
...
log_info "TDI-6.1 reproduction completed"
