#!/usr/bin/env bash
#
# TDI-5.4 reproduction script — nonlinear sufficiency and horizon-invariance
# of the O1/O2 asymmetry. This script performs the real, preregistered
# 120,000-record TDI-5.4 run exactly once.
#
...
TDI54_CONFIRM_VAR="TDI54_CONFIRM_FULL_RUN"
TDI54_CONFIRM_VALUE="I_ACCEPT_THE_TDI54_FREEZE_RULE"

PREREG_HASH="docs/TDI-5.4-NONLINEAR-OVERLAP-SUFFICIENCY-PREREGISTRATION.sha256"
EVALUATOR_HASH="docs/TDI-5.4-NONLINEAR-OVERLAP-SUFFICIENCY-EVALUATOR.sha256"
SCIENTIFIC_HASH="docs/TDI-5.4-SCIENTIFIC-CODE.sha256"
...
RESULT_DIR="results/tdi5.4-nonlinear-overlap-sufficiency"
RESULT_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v54.log"
METADATA_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v54.metadata.txt"
RESULT_HASH_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v54.log.sha256"
COMPLETION_MARKER="${RESULT_DIR}/tdi-independent-overlap-ablation-v54.complete"
LOCK_DIR="${RESULT_DIR}/.tdi5.4.lock"

BINARY_NAME="tdi-independent-overlap-ablation-v54"
...
require_full_run_confirmation() {
    local -r actual="${!TDI54_CONFIRM_VAR:-}"

    if [[ "$actual" != "$TDI54_CONFIRM_VALUE" ]]; then
        log_error "refusing: ${TDI54_CONFIRM_VAR} must be set to the exact value ${TDI54_CONFIRM_VALUE}"
        log_error "the real command is:"
        log_error "  ${TDI54_CONFIRM_VAR}=${TDI54_CONFIRM_VALUE} bash ${BASH_SOURCE[0]}"
        exit 1
    fi
}
...
require_clean_git() {
    if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
        log_error "repository must be clean before the full TDI-5.4 evaluation"
        git status --short
        exit 1
    fi
}
...
refuse_existing_output() {
    if [[ -e "$COMPLETION_MARKER" ]]; then
        log_error "a completed TDI-5.4 run already exists"
    ...
    if [[ -e "$RESULT_FILE" || -e "$RESULT_HASH_FILE" || -e "$METADATA_FILE" ]]; then
        log_error "incomplete TDI-5.4 output exists; refusing to overwrite it"
        ls -lh -- "$RESULT_DIR" >&2
        exit 1
    fi
}
...
acquire_lock() {
    ...
    if ! mkdir -- "$LOCK_DIR" 2>/dev/null; then
        log_error "another TDI-5.4 reproduction appears to be running: ${LOCK_DIR}"
        exit 1
    fi
...
write_initial_metadata() {
    ...
    {
        printf 'experiment=TDI-5.4 nonlinear overlap sufficiency\n'
        printf 'start_utc=%s\n' "$start_timestamp"
        printf 'commit=%s\n' "$(git rev-parse HEAD)"
        printf 'repository=%s\n' "$ROOT"
        ...
        printf 'preregistration_sha256=%s\n' "$(sha256sum "docs/TDI-5.4-NONLINEAR-OVERLAP-SUFFICIENCY-PREREGISTRATION.md" | awk '{print $1}')"
        ...
        printf 'frozen_tdi53_evaluator_sha256=%s\n' "$(sha256sum "tdi-bench/src/bin/tdi-independent-overlap-ablation-v53.rs" | awk '{print $1}')"
        printf 'frozen_tdi53_preregistration_sha256=%s\n' "$(sha256sum "docs/TDI-5.3-INDEPENDENT-OVERLAP-ACTIVATION-PREREGISTRATION.md" | awk '{print $1}')"
    } > "$METADATA_FILE"
}
...
verify_complete_output() {
    ...
    # TDI-5.4 preregistration Section 15 requires the final TDI-5.4A
    # classification and the TDI-5.4B per-horizon classifications and
    # invariance summary; Section 17 requires verifying all final criterion
    # lines, so every phrase below must be present.
...
log_info "verifying TDI-5.4 hashes"
...
if [[ "$RUN_STATUS" -ne 0 ]]; then
    log_error "TDI-5.4 evaluator failed with status ${RUN_STATUS}"
    exit "$RUN_STATUS"
fi
...
log_info "TDI-5.4 reproduction completed"
