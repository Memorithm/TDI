#!/usr/bin/env bash
#
# TDI-6.7 reproduction script — generator-family robustness of the literal-
# spectral control (does the GKT-vs-GK overlap signal — the overlaps' value
# beyond contraction, the exact spectral moments, AND the literal spectral gap
# g = 1 − |λ₂| plus the ε-mixing time τ_ε — replicate across the frozen TDI-5.7
# generator family F0–F3, rather than on the single base generator of TDI-6.1?).
# One factor changed vs 6.1: the generator (single → the four exact families of
# 5.7). The two non-exact spectral descriptors are inherited unchanged from 6.1,
# so this stays the tolerance-based (non-byte-exact across architectures) regime
# of Section 13. This script performs the real, preregistered 480,000-record
# TDI-6.7 run (4 families × 3 blocks × 40,000 accepted records) exactly once.
#
...
#
# Reproduction is tolerance-based, not byte-exact (Section 13): on the
...
# metrics may differ in the last f64 digits, but the printed 6.7A/6.7B/6.7C/6.7D
...
TDI67_CONFIRM_VAR="TDI67_CONFIRM_FULL_RUN"
TDI67_CONFIRM_VALUE="I_ACCEPT_THE_TDI67_FREEZE_RULE"

PREREG_HASH="docs/TDI-6.7-OBSERVABLE-OFFSET-TRANSFER-PREREGISTRATION.sha256"
EVALUATOR_HASH="docs/TDI-6.7-OBSERVABLE-OFFSET-TRANSFER-EVALUATOR.sha256"
SCIENTIFIC_HASH="docs/TDI-6.7-SCIENTIFIC-CODE.sha256"
...
# TDI-5.8, TDI-5.9 and TDI-6.5 are frozen ancestors of TDI-6.7; TDI-6.5 is its
...
# TDI-6.6 is TDI-6.7's scientific ancestor: the evaluator this one derives from
# by a single changed factor (preregistration Section 1).
...
RESULT_DIR="results/tdi6.7-observable-offset-transfer"
RESULT_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v67.log"
METADATA_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v67.metadata.txt"
RESULT_HASH_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v67.log.sha256"
COMPLETION_MARKER="${RESULT_DIR}/tdi-independent-overlap-ablation-v67.complete"
LOCK_DIR="${RESULT_DIR}/.tdi6.7.lock"

BINARY_NAME="tdi-independent-overlap-ablation-v67"
...
require_full_run_confirmation() {
    local -r actual="${!TDI67_CONFIRM_VAR:-}"

    if [[ "$actual" != "$TDI67_CONFIRM_VALUE" ]]; then
        log_error "refusing: ${TDI67_CONFIRM_VAR} must be set to the exact value ${TDI67_CONFIRM_VALUE}"
        log_error "the real command is:"
        log_error "  ${TDI67_CONFIRM_VAR}=${TDI67_CONFIRM_VALUE} bash ${BASH_SOURCE[0]}"
        exit 1
    fi
}
...
require_clean_git() {
    if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
        log_error "repository must be clean before the full TDI-6.7 evaluation"
        git status --short
        exit 1
    fi
}
...
refuse_existing_output() {
    if [[ -e "$COMPLETION_MARKER" ]]; then
        log_error "a completed TDI-6.7 run already exists"
    ...
    if [[ -e "$RESULT_FILE" || -e "$RESULT_HASH_FILE" || -e "$METADATA_FILE" ]]; then
        log_error "incomplete TDI-6.7 output exists; refusing to overwrite it"
        ls -lh -- "$RESULT_DIR" >&2
        exit 1
    fi
}
...
acquire_lock() {
    ...
    if ! mkdir -- "$LOCK_DIR" 2>/dev/null; then
        log_error "another TDI-6.7 reproduction appears to be running: ${LOCK_DIR}"
        exit 1
    fi
...
write_initial_metadata() {
    ...
    {
        printf 'experiment=TDI-6.7 observable-offset cross-generator transfer\n'
        printf 'start_utc=%s\n' "$start_timestamp"
        printf 'commit=%s\n' "$(git rev-parse HEAD)"
        printf 'repository=%s\n' "$ROOT"
...
verify_complete_output() {
    ...
    # TDI-6.7 preregistration Sections 21-23 require the final 6.7A/6.7B/6.7C/6.7D
...
log_info "verifying frozen TDI-6.6 hashes (scientific ancestor)"
sha256sum -c "$FROZEN_TDI66_PREREG_HASH"
sha256sum -c "$FROZEN_TDI66_EVALUATOR_HASH"

log_info "verifying TDI-6.7 hashes"
...
if [[ "$RUN_STATUS" -ne 0 ]]; then
    log_error "TDI-6.7 evaluator failed with status ${RUN_STATUS}"
    exit "$RUN_STATUS"
fi
...
log_info "TDI-6.7 reproduction completed"
