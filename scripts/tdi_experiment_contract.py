...
def _u64(value, name, *, positive=False):
    if type(value) is not int or value < (1 if positive else 0) or value > UINT64_MAX:
        ...
        raise ExperimentContractError(f"{name} must be a {qualifier}u64 integer")
    return value
...
def validate_experiment_spec(spec):
    ...
    The schema contains no implicit defaults. Scientific meaning remains owned by
    the series protocol; this validates identity structure and operational bounds.
    """
    _exact(
        spec,
        {
    ...
    data = _exact(
        spec["data"],
        {"access_class", "generator_identity", "partition_derivation"},
        "data",
    )
    if data["access_class"] not in (
        "public",
        "development",
        "validation",
    ...
    randomness = _exact(
        spec["randomness"],
        {"algorithm", "seed_namespace", "stream_derivation"},
        "randomness",
    )
    ...
    _optional_u64(
        physical["gpu_memory_max_bytes"],
        "physical_constraints.gpu_memory_max_bytes",
        positive=True,
    )
    ...
    statistics = _exact(
        spec["statistics"],
        {"plan_identity", "missing_observation_policy"},
        "statistics",
    )
    ...
    artifact_policy = _exact(
        spec["artifacts"],
        {"access_class", "retention_policy", "export_policy"},
        "artifacts",
    )
...
def question_identity(spec):
    """Return the scientific question/protocol identity.

    Physical execution constraints, adapter implementation, arms and retry state
    are intentionally excluded. A semantic protocol/metric/data/statistics change
    ...
    value = {
        ...
        "stage": spec["stage"],
        "protocol_refs": spec["protocol_refs"],
...
def stream_identity(question_id, trial_id, namespace, stream):
    """Return a stable RNG stream coordinate, not a random sample or proof of independence."""
...
def parse_seed_decimal(value):
    ...
    Strings prevent JavaScript clients from rounding values above 2**53-1.
    """
...
def validate_worker_response_v2(
    response,
    *,
    experiment_id,
    plan_id,
...
def scientific_result_identity(response):
    """Hash canonical scientific output while excluding variable operational telemetry."""
    ...
    return _digest("tdi-scientific-result/v1", value)
