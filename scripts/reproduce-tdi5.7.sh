#!/usr/bin/env bash
#
# TDI-5.7 reproduction script — generator robustness (does the SKT-vs-SK
# overlap signal replicate across a family of exact generators?). This script
# performs the real, preregistered 480,000-record TDI-5.7 run (4 families ×
...
TDI57_CONFIRM_VAR="TDI57_CONFIRM_FULL_RUN"
TDI57_CONFIRM_VALUE="I_ACCEPT_THE_TDI57_FREEZE_RULE"

PREREG_HASH="docs/TDI-5.7-GENERATOR-ROBUSTNESS-PREREGISTRATION.sha256"
EVALUATOR_HASH="docs/TDI-5.7-GENERATOR-ROBUSTNESS-EVALUATOR.sha256"
SCIENTIFIC_HASH="docs/TDI-5.7-SCIENTIFIC-CODE.sha256"
...
RESULT_DIR="results/tdi5.7-generator-robustness"
RESULT_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v57.log"
METADATA_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v57.metadata.txt"
RESULT_HASH_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v57.log.sha256"
COMPLETION_MARKER="${RESULT_DIR}/tdi-independent-overlap-ablation-v57.complete"
LOCK_DIR="${RESULT_DIR}/.tdi5.7.lock"

BINARY_NAME="tdi-independent-overlap-ablation-v57"
...
require_full_run_confirmation() {
    local -r actual="${!TDI57_CONFIRM_VAR:-}"

    if [[ "$actual" != "$TDI57_CONFIRM_VALUE" ]]; then
        log_error "refusing: ${TDI57_CONFIRM_VAR} must be set to the exact value ${TDI57_CONFIRM_VALUE}"
        log_error "the real command is:"
        log_error "  ${TDI57_CONFIRM_VAR}=${TDI57_CONFIRM_VALUE} bash ${BASH_SOURCE[0]}"
        exit 1
    fi
}
...
require_clean_git() {
    if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
        log_error "repository must be clean before the full TDI-5.7 evaluation"
        git status --short
        exit 1
    fi
}
...
refuse_existing_output() {
    if [[ -e "$COMPLETION_MARKER" ]]; then
        log_error "a completed TDI-5.7 run already exists"
    ...
    if [[ -e "$RESULT_FILE" || -e "$RESULT_HASH_FILE" || -e "$METADATA_FILE" ]]; then
        log_error "incomplete TDI-5.7 output exists; refusing to overwrite it"
        ls -lh -- "$RESULT_DIR" >&2
        exit 1
    fi
}
...
acquire_lock() {
    ...
    if ! mkdir -- "$LOCK_DIR" 2>/dev/null; then
        log_error "another TDI-5.7 reproduction appears to be running: ${LOCK_DIR}"
        exit 1
    fi
...
write_initial_metadata() {
    ...
    {
        printf 'experiment=TDI-5.7 generator robustness\n'
        printf 'start_utc=%s\n' "$start_timestamp"
        printf 'commit=%s\n' "$(git rev-parse HEAD)"
        printf 'repository=%s\n' "$ROOT"
...
verify_complete_output() {
    ...
    # TDI-5.7 preregistration Section 17 requires the final TDI-5.7A
    # replication verdict, the TDI-5.7B heterogeneity summary, the TDI-5.7C
    # transfer classification and the TDI-5.7D descriptor-drift table, so
    # every phrase below must be present.
...
log_info "verifying TDI-5.7 hashes"
...
if [[ "$RUN_STATUS" -ne 0 ]]; then
    log_error "TDI-5.7 evaluator failed with status ${RUN_STATUS}"
    exit "$RUN_STATUS"
fi
...
log_info "TDI-5.7 reproduction completed"
