#!/usr/bin/env bash
#
# TDI-6.5 reproduction script — generator-family robustness of the literal-
...
# TDI-6.5 run (4 families × 3 blocks × 40,000 accepted records) exactly once.
#
...
# metrics may differ in the last f64 digits, but the printed 6.5A/6.5B/6.5C/6.5D
...
TDI65_CONFIRM_VAR="TDI65_CONFIRM_FULL_RUN"
TDI65_CONFIRM_VALUE="I_ACCEPT_THE_TDI65_FREEZE_RULE"

PREREG_HASH="docs/TDI-6.5-GENERATOR-FAMILY-SPECTRAL-ROBUSTNESS-PREREGISTRATION.sha256"
EVALUATOR_HASH="docs/TDI-6.5-GENERATOR-FAMILY-SPECTRAL-ROBUSTNESS-EVALUATOR.sha256"
SCIENTIFIC_HASH="docs/TDI-6.5-SCIENTIFIC-CODE.sha256"
...
RESULT_DIR="results/tdi6.5-family-spectral-robustness"
RESULT_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v65.log"
METADATA_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v65.metadata.txt"
RESULT_HASH_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v65.log.sha256"
COMPLETION_MARKER="${RESULT_DIR}/tdi-independent-overlap-ablation-v65.complete"
LOCK_DIR="${RESULT_DIR}/.tdi6.5.lock"

BINARY_NAME="tdi-independent-overlap-ablation-v65"
...
require_full_run_confirmation() {
    local -r actual="${!TDI65_CONFIRM_VAR:-}"

    if [[ "$actual" != "$TDI65_CONFIRM_VALUE" ]]; then
        log_error "refusing: ${TDI65_CONFIRM_VAR} must be set to the exact value ${TDI65_CONFIRM_VALUE}"
        log_error "the real command is:"
        log_error "  ${TDI65_CONFIRM_VAR}=${TDI65_CONFIRM_VALUE} bash ${BASH_SOURCE[0]}"
        exit 1
    fi
}
...
require_clean_git() {
    if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
        log_error "repository must be clean before the full TDI-6.5 evaluation"
        git status --short
        exit 1
    fi
}
...
refuse_existing_output() {
    if [[ -e "$COMPLETION_MARKER" ]]; then
        log_error "a completed TDI-6.5 run already exists"
    ...
    if [[ -e "$RESULT_FILE" || -e "$RESULT_HASH_FILE" || -e "$METADATA_FILE" ]]; then
        log_error "incomplete TDI-6.5 output exists; refusing to overwrite it"
        ls -lh -- "$RESULT_DIR" >&2
        exit 1
    fi
}
...
acquire_lock() {
    ...
    if ! mkdir -- "$LOCK_DIR" 2>/dev/null; then
        log_error "another TDI-6.5 reproduction appears to be running: ${LOCK_DIR}"
        exit 1
    fi
...
write_initial_metadata() {
    ...
    {
        printf 'experiment=TDI-6.5 generator-family robustness of the literal-spectral control\n'
        printf 'start_utc=%s\n' "$start_timestamp"
        printf 'commit=%s\n' "$(git rev-parse HEAD)"
        printf 'repository=%s\n' "$ROOT"
        ...
        printf 'preregistration_sha256=%s\n' "$(sha256sum "docs/TDI-6.5-GENERATOR-FAMILY-SPECTRAL-ROBUSTNESS-PREREGISTRATION.md" | awk '{print $1}')"
...
verify_complete_output() {
    ...
    # TDI-6.5 preregistration Sections 21-23 require the final 6.5A/6.5B/6.5C/6.5D
...
log_info "verifying TDI-6.5 hashes"
...
if [[ "$RUN_STATUS" -ne 0 ]]; then
    log_error "TDI-6.5 evaluator failed with status ${RUN_STATUS}"
    exit "$RUN_STATUS"
fi
...
log_info "TDI-6.5 reproduction completed"
