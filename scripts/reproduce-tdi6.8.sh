#!/usr/bin/env bash
#
# TDI-6.8 reproduction script — transportable ordering (does the overlap improve
# *transferred ordering*, and does the ordering transfer at all?).
#
# Four experiments in a row found cross-generator transfer of the deficit LEVEL
# to be broken, and each time the reading offered was that the ORDERING might
# still survive. That reading was asserted four times and tested zero times.
# TDI-6.8 tests it: one arm (plain transfer, no correction of any kind), four
# layouts (CK/SK/GK/GKT), and a per-seed-block Spearman rho — never pooled,
# because pooling blocks whose deficit levels differ would rank across domains
# and manufacture correlation from the level gap alone.
#
# One factor changed vs TDI-6.7: what is measured — a rank statistic instead of
# a level statistic. The two non-exact spectral descriptors are inherited
# unchanged, so this stays the tolerance-based (non-byte-exact across
# architectures) regime. Ranks of finite doubles are exact and add none.
#
# This script performs the real, preregistered 480,000-record TDI-6.8 run
# (4 families x 3 blocks x 40,000 accepted records) exactly once.
#
...
#
# Reproduction is tolerance-based, not byte-exact: on the reference
# toolchain/architecture the result log reproduces byte-for-byte and the
# completion check verifies its SHA-256; across architectures the raw metrics
# may differ in the last f64 digits, but the printed 6.8A/6.8B/6.8C/6.8D
...
TDI68_CONFIRM_VAR="TDI68_CONFIRM_FULL_RUN"
TDI68_CONFIRM_VALUE="I_ACCEPT_THE_TDI68_FREEZE_RULE"

PREREG_HASH="docs/TDI-6.8-TRANSPORTABLE-ORDERING-PREREGISTRATION.sha256"
EVALUATOR_HASH="docs/TDI-6.8-TRANSPORTABLE-ORDERING-EVALUATOR.sha256"
SCIENTIFIC_HASH="docs/TDI-6.8-SCIENTIFIC-CODE.sha256"
...
# TDI-5.8, TDI-5.9 and TDI-6.5 are frozen ancestors of TDI-6.8; TDI-6.5 is its
# scientific ancestor, the evaluator this one derives from by a single changed
# factor (preregistration Section 1).
...
FROZEN_TDI59_PREREG_HASH="docs/TDI-5.9-SPECTRAL-MOMENT-SATURATION-PREREGISTRATION.sha256"
FROZEN_TDI59_EVALUATOR_HASH="docs/TDI-5.9-SPECTRAL-MOMENT-SATURATION-EVALUATOR.sha256"
...
FROZEN_TDI66_PREREG_HASH="docs/TDI-6.6-RESTANDARDIZED-TRANSFER-PREREGISTRATION.sha256"
FROZEN_TDI66_EVALUATOR_HASH="docs/TDI-6.6-RESTANDARDIZED-TRANSFER-EVALUATOR.sha256"
FROZEN_TDI66_SCIENTIFIC_HASH="docs/TDI-6.6-SCIENTIFIC-CODE.sha256"

# TDI-6.8 is TDI-6.8's scientific ancestor: the evaluator this one derives from
# by a single changed factor — what is measured (preregistration Section 1).
FROZEN_TDI67_PREREG_HASH="docs/TDI-6.7-OBSERVABLE-OFFSET-TRANSFER-PREREGISTRATION.sha256"
FROZEN_TDI67_EVALUATOR_HASH="docs/TDI-6.7-OBSERVABLE-OFFSET-TRANSFER-EVALUATOR.sha256"
FROZEN_TDI67_SCIENTIFIC_HASH="docs/TDI-6.7-SCIENTIFIC-CODE.sha256"

RESULT_DIR="results/tdi6.8-transportable-ordering"
RESULT_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v68.log"
METADATA_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v68.metadata.txt"
RESULT_HASH_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v68.log.sha256"
COMPLETION_MARKER="${RESULT_DIR}/tdi-independent-overlap-ablation-v68.complete"
LOCK_DIR="${RESULT_DIR}/.tdi6.8.lock"

BINARY_NAME="tdi-independent-overlap-ablation-v68"
...
require_full_run_confirmation() {
    local -r actual="${!TDI68_CONFIRM_VAR:-}"

    if [[ "$actual" != "$TDI68_CONFIRM_VALUE" ]]; then
        log_error "refusing: ${TDI68_CONFIRM_VAR} must be set to the exact value ${TDI68_CONFIRM_VALUE}"
        log_error "the real command is:"
        log_error "  ${TDI68_CONFIRM_VAR}=${TDI68_CONFIRM_VALUE} bash ${BASH_SOURCE[0]}"
        exit 1
    fi
}
...
require_clean_git() {
    if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
        log_error "repository must be clean before the full TDI-6.8 evaluation"
        git status --short
        exit 1
    fi
}
...
refuse_existing_output() {
    if [[ -e "$COMPLETION_MARKER" ]]; then
        log_error "a completed TDI-6.8 run already exists"
    ...
    if [[ -e "$RESULT_FILE" || -e "$RESULT_HASH_FILE" || -e "$METADATA_FILE" ]]; then
        log_error "incomplete TDI-6.8 output exists; refusing to overwrite it"
        ls -lh -- "$RESULT_DIR" >&2
        exit 1
    fi
}
...
acquire_lock() {
    ...
    if ! mkdir -- "$LOCK_DIR" 2>/dev/null; then
        log_error "another TDI-6.8 reproduction appears to be running: ${LOCK_DIR}"
        exit 1
    fi
...
write_initial_metadata() {
    ...
    {
        printf 'experiment=TDI-6.8 transportable ordering\n'
        printf 'start_utc=%s\n' "$start_timestamp"
        printf 'commit=%s\n' "$(git rev-parse HEAD)"
        printf 'repository=%s\n' "$ROOT"
        ...
        printf 'preregistration_sha256=%s\n' "$(sha256sum "docs/TDI-6.8-OBSERVABLE-OFFSET-TRANSFER-PREREGISTRATION.md" | awk '{print $1}')"
        printf 'scientific_manifest_sha256=%s\n' "$(sha256sum "$SCIENTIFIC_HASH" | awk '{print $1}')"
        printf 'frozen_tdi67_evaluator_sha256=%s\n' "$(sha256sum "tdi-bench/src/bin/tdi-independent-overlap-ablation-v67.rs" | awk '{print $1}')"
        printf 'frozen_tdi67_preregistration_sha256=%s\n' "$(sha256sum "docs/TDI-6.7-OBSERVABLE-OFFSET-TRANSFER-PREREGISTRATION.md" | awk '{print $1}')"
        ...
        printf 'frozen_tdi57_preregistration_sha256=%s\n' "$(sha256sum "docs/TDI-5.7-GENERATOR-ROBUSTNESS-PREREGISTRATION.md" | awk '{print $1}')"
...
verify_complete_output() {
    ...
    # TDI-6.8 preregistration Sections 21-23 require the final 6.7A/6.7B/6.7C/6.7D
    # classification lines (the tolerance-robust invariant of Section 13), so
...
log_info "verifying frozen TDI-5.9 hashes"
sha256sum -c "$FROZEN_TDI59_PREREG_HASH"
sha256sum -c "$FROZEN_TDI59_EVALUATOR_HASH"

log_info "verifying frozen TDI-6.5 hashes (scientific ancestor)"
sha256sum -c "$FROZEN_TDI65_PREREG_HASH"
sha256sum -c "$FROZEN_TDI65_EVALUATOR_HASH"

log_info "verifying frozen TDI-6.6 hashes"
sha256sum -c "$FROZEN_TDI66_PREREG_HASH"
sha256sum -c "$FROZEN_TDI66_EVALUATOR_HASH"
sha256sum -c "$FROZEN_TDI66_SCIENTIFIC_HASH"

log_info "verifying frozen TDI-6.7 hashes (scientific ancestor)"
sha256sum -c "$FROZEN_TDI67_PREREG_HASH"
sha256sum -c "$FROZEN_TDI67_EVALUATOR_HASH"
sha256sum -c "$FROZEN_TDI67_SCIENTIFIC_HASH"

log_info "verifying TDI-6.8 hashes"
...
if [[ "$RUN_STATUS" -ne 0 ]]; then
    log_error "TDI-6.8 evaluator failed with status ${RUN_STATUS}"
    exit "$RUN_STATUS"
fi
...
log_info "TDI-6.8 reproduction completed"
