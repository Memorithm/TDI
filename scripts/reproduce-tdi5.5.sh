#!/usr/bin/env bash
#
# TDI-5.5 reproduction script — the baseline challenge (contraction and
# persistence confounds). This script performs the real, preregistered
# 120,000-record TDI-5.5 run exactly once.
#
...
TDI55_CONFIRM_VAR="TDI55_CONFIRM_FULL_RUN"
TDI55_CONFIRM_VALUE="I_ACCEPT_THE_TDI55_FREEZE_RULE"

PREREG_HASH="docs/TDI-5.5-OVERLAP-BASELINE-CHALLENGE-PREREGISTRATION.sha256"
EVALUATOR_HASH="docs/TDI-5.5-OVERLAP-BASELINE-CHALLENGE-EVALUATOR.sha256"
SCIENTIFIC_HASH="docs/TDI-5.5-SCIENTIFIC-CODE.sha256"
...
RESULT_DIR="results/tdi5.5-overlap-baseline-challenge"
RESULT_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v55.log"
METADATA_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v55.metadata.txt"
RESULT_HASH_FILE="${RESULT_DIR}/tdi-independent-overlap-ablation-v55.log.sha256"
COMPLETION_MARKER="${RESULT_DIR}/tdi-independent-overlap-ablation-v55.complete"
LOCK_DIR="${RESULT_DIR}/.tdi5.5.lock"

BINARY_NAME="tdi-independent-overlap-ablation-v55"
...
require_full_run_confirmation() {
    local -r actual="${!TDI55_CONFIRM_VAR:-}"

    if [[ "$actual" != "$TDI55_CONFIRM_VALUE" ]]; then
        log_error "refusing: ${TDI55_CONFIRM_VAR} must be set to the exact value ${TDI55_CONFIRM_VALUE}"
        log_error "the real command is:"
        log_error "  ${TDI55_CONFIRM_VAR}=${TDI55_CONFIRM_VALUE} bash ${BASH_SOURCE[0]}"
        exit 1
    fi
}
...
require_clean_git() {
    if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
        log_error "repository must be clean before the full TDI-5.5 evaluation"
        git status --short
        exit 1
    fi
}
...
refuse_existing_output() {
    if [[ -e "$COMPLETION_MARKER" ]]; then
        log_error "a completed TDI-5.5 run already exists"
    ...
    if [[ -e "$RESULT_FILE" || -e "$RESULT_HASH_FILE" || -e "$METADATA_FILE" ]]; then
        log_error "incomplete TDI-5.5 output exists; refusing to overwrite it"
        ls -lh -- "$RESULT_DIR" >&2
        exit 1
    fi
}
...
acquire_lock() {
    ...
    if ! mkdir -- "$LOCK_DIR" 2>/dev/null; then
        log_error "another TDI-5.5 reproduction appears to be running: ${LOCK_DIR}"
        exit 1
    fi
...
write_initial_metadata() {
    ...
    {
        printf 'experiment=TDI-5.5 overlap baseline challenge\n'
        printf 'start_utc=%s\n' "$start_timestamp"
        printf 'commit=%s\n' "$(git rev-parse HEAD)"
        printf 'repository=%s\n' "$ROOT"
...
verify_complete_output() {
    ...
    # TDI-5.5 preregistration Section 17 requires the final TDI-5.5A and
    # TDI-5.5B focal classifications and the TDI-5.5C decay-law summary, and
...
log_info "verifying TDI-5.5 hashes"
...
if [[ "$RUN_STATUS" -ne 0 ]]; then
    log_error "TDI-5.5 evaluator failed with status ${RUN_STATUS}"
    exit "$RUN_STATUS"
fi
...
log_info "TDI-5.5 reproduction completed"
