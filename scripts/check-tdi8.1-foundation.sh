...
if test -f tdi-ai/src/task_encoding.rs; then
    test -s scripts/check-tdi8.1-task-encoding.sh \
        || fail "task encoding candidate exists without its promotion/integrity gate"
    bash scripts/check-tdi8.1-task-encoding.sh
fi
...
printf 'TDI-8.1 bounded task-encoding promotion gate: CHAINED\n'
