...
if test -f tdi-ai/src/task_readout.rs; then
    test -s scripts/check-tdi8.1-exact-readout.sh \
        || fail "exact recurrent readout candidate exists without its integrity gate"
    bash scripts/check-tdi8.1-exact-readout.sh
fi

if test -f tdi-ai/src/bin/tdi8-a0-a1-adapter-preflight/main.rs; then
    test -s scripts/check-tdi8.1-a0-a1-adapter.sh \
        || fail "A0/A1 adapter preflight exists without its integrity gate"
    bash scripts/check-tdi8.1-a0-a1-adapter.sh
fi

if test -f tdi-ai/src/bin/tdi8-a2-adapter-preflight/main.rs; then
    test -s scripts/check-tdi8.1-a2-adapter.sh \
        || fail "A2 adapter preflight exists without its integrity gate"
...
if test -f tdi-bench/src/paired_resampling_v8.rs; then
    test -s scripts/check-tdi8.1-paired-resampling.sh \
        || fail "TDI-8 paired-resampling source exists without its integrity gate"
