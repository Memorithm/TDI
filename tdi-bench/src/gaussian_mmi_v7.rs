//! Deterministic Gaussian/MMI information decomposition for bounded TDI-7.x research.
//!
//! The implementation generalizes the TDI-6.3 Gaussian/MMI discipline from two
//! scalar sources to finite vector blocks. It contains no final-holdout execution
//! or authorization surface and produces no confirmatory classification.
...
fn observation_width(rows: &[Vec<f64>]) -> Result<usize, InformationError> {
...
fn square_width(matrix: &[Vec<f64>]) -> Result<usize, InformationError> {
    let first = matrix.first().ok_or(InformationError::EmptyDataset)?;
    let width = first.len();
    if width == 0 || matrix.len() != width {
        return Err(InformationError::FeatureWidthMismatch);
    }
    for row in matrix {
        if row.len() != width {
...
fn validate_records(records: &[InformationRecord]) -> Result<(), InformationError> {
...
fn reduce_rank(rows: &[Vec<f64>]) -> Result<ReducedBlock, InformationError> {
    let width = observation_width(rows)?;
    let n = rows.len();
    let threshold = PID_RANK_RESIDUAL_SCALE * (n as f64).sqrt();
    let mut standardized_columns = Vec::<Vec<f64>>::new();
    ...
    for column in 0..width {
        ...
        let mut values = rows
            ...
            .collect::<Vec<_>>();
        let rms = (values.iter().map(|value| value * value).sum::<f64>() / n as f64).sqrt();
        if !rms.is_finite() {
            return Err(InformationError::NonFiniteValue);
        }
        if rms == 0.0 {
        ...
        for direction in &basis {
            let projection = residual
                ...
                .sum::<f64>();
            for (value, direction_value) in residual.iter_mut().zip(direction) {
    ...
    let rank = standardized_columns.len();
    if rank == 0 {
        return Err(InformationError::ZeroSourceRank);
    }
    let mut reduced_rows = vec![vec![0.0; rank]; n];
    for (column, values) in standardized_columns.iter().enumerate() {
...
fn variance(values: &[f64]) -> Result<f64, InformationError> {
    if values.len() < 3 {
        return Err(InformationError::TooFewRecords);
    }
    ...
    if value <= PID_DEGENERACY_PIVOT_FLOOR {
...
fn target_source_covariance(
    ...
    let width = observation_width(source)?;
    ...
    let mut covariance = Vec::with_capacity(width);
    for column in 0..width {
        let value = target
            .iter()
            .zip(source)
            .map(|(target_value, row)| {
                (target_value - target_mean) * (row[column] - source_means[column])
            })
            .sum::<f64>()
            / denominator;
        if !value.is_finite() {
...
fn cholesky_log2_determinant(matrix: &[Vec<f64>]) -> Result<f64, InformationError> {
    ...
    for i in 0..n {
        for j in 0..=i {
            ...
            if i == j {
                ...
                lower[i][j] = (matrix[i][j] - product_sum) / denominator;
    ...
    let log2_det = 2.0 * (0..n).map(|index| lower[index][index].log2()).sum::<f64>();
    if log2_det.is_finite() {
        Ok(log2_det)
    } else {
...
fn normalize_information(value: f64) -> Result<f64, InformationError> {
    if !value.is_finite() || value < -PID_CROSS_METHOD_TOLERANCE_BITS {
        return Err(InformationError::InvalidInformation);
    }
    Ok(if value < 0.0 { 0.0 } else { value })
}

fn canonical_mi(target: &[f64], source: &ReducedBlock) -> Result<f64, InformationError> {
    let target_variance = variance(target)?;
    let source_covariance = covariance_matrix(&source.rows)?;
    let source_logdet = cholesky_log2_determinant(&source_covariance)?;
    ...
    let joint_logdet = cholesky_log2_determinant(&covariance_matrix(&joint_rows)?)?;
    normalize_information(0.5 * (target_variance.log2() + source_logdet - joint_logdet))
}

fn crosscheck_mi(target: &[f64], source: &ReducedBlock) -> Result<f64, InformationError> {
    let target_variance = variance(target)?;
    let source_covariance = covariance_matrix(&source.rows)?;
...
pub fn gaussian_mutual_information(
    ...
    let canonical_bits = canonical_mi(target, &reduced)?;
    let crosscheck_bits = crosscheck_mi(target, &reduced)?;
...
fn nonnegative_pid(value: f64) -> Result<f64, InformationError> {
    if !value.is_finite() {
        return Err(InformationError::InvalidInformation);
    }
...
pub fn evaluate_pid(records: &[InformationRecord]) -> Result<PidEstimate, InformationError> {
    ...
    let unique_static_bits = nonnegative_pid(static_mi.canonical_bits - redundancy_bits)?;
    let unique_recovery_bits = nonnegative_pid(recovery_mi.canonical_bits - redundancy_bits)?;
    let synergy_bits = nonnegative_pid(
...
fn bootstrap_degeneracy(error: InformationError) -> bool {
    matches!(
        error,
...
pub fn bootstrap_pid(
    ...
    let mut joint = Vec::with_capacity(replicates);
    let mut redundancy = Vec::with_capacity(replicates);
    let mut unique_static = Vec::with_capacity(replicates);
    let mut unique_recovery = Vec::with_capacity(replicates);
    let mut synergy = Vec::with_capacity(replicates);
    ...
    for _ in 0..replicates {
        ...
        match evaluate_pid(&resampled) {
            Ok(estimate) => {
                joint.push(estimate.joint_mi.canonical_bits);
                redundancy.push(estimate.redundancy_bits);
                unique_static.push(estimate.unique_static_bits);
                unique_recovery.push(estimate.unique_recovery_bits);
                synergy.push(estimate.synergy_bits);
            }
            Err(error) if bootstrap_degeneracy(error) => {
    ...
    let accepted_replicates = joint.len();
    ...
    Ok(PidBootstrapSummary {
        point,
        joint: interval(joint)?,
        redundancy: interval(redundancy)?,
        unique_static: interval(unique_static)?,
        unique_recovery: interval(unique_recovery)?,
        synergy: interval(synergy)?,
        accepted_replicates,
        rejected_replicates,
    })
}
...
mod tests {
            ...
            for site in 0..2 {
                ...
                records.push(
                    InformationRecord::new(
                        ...
                        0.31 * x - 0.17 * z + 0.08 * w + site_residual,
                    )
                    .unwrap(),
                );
            }
    ...
    #[test]
    fn cholesky_supports_one_dimensional_covariance() {
        let value = cholesky_log2_determinant(&[vec![4.0]]).unwrap();
    ...
    fn scalar_source_matches_closed_form_correlation_identity() {
        ...
        let target_norm = target
            ...
            .sum::<f64>();
        let source_norm = source_values
            .iter()
            .map(|value| (value - source_mean).powi(2))
            .sum::<f64>();
        let rho_squared = numerator * numerator / (target_norm * source_norm);
    ...
    #[test]
    fn duplicate_columns_are_removed_without_changing_information() {
        let target = vec![-2.0, -1.0, 0.5, 1.0, 2.5, 4.0, 5.0, 7.0];
        let values = vec![-3.0, -1.5, -0.5, 0.5, 1.5, 2.0, 4.0, 5.0];
        let one = values.iter().map(|value| vec![*value]).collect::<Vec<_>>();
        let duplicated = values
            .iter()
            .map(|value| vec![*value, *value, 2.0 * *value])
            .collect::<Vec<_>>();
        let left = gaussian_mutual_information(&target, &one).unwrap();
        let right = gaussian_mutual_information(&target, &duplicated).unwrap();
    ...
    #[test]
    fn vector_pid_satisfies_identity_and_cross_method_tolerance() {
        let estimate = evaluate_pid(&synthetic_records(48)).unwrap();
        ...
        for mi in [estimate.static_mi(), estimate.recovery_mi(), estimate.joint_mi()] {
            assert!(mi.discrepancy_bits() <= PID_CROSS_METHOD_TOLERANCE_BITS);
        }
    }
    ...
    #[test]
    fn malformed_widths_fail_closed() {
