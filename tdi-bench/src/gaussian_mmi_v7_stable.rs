//! Numerically stable bounded Gaussian/MMI information decomposition for TDI-7.x.
//!
//! The estimator works on the orthonormal basis actually produced by rank
//! reduction. This preserves the Gaussian mutual information of the source
//! subspace while avoiding the ill-conditioning caused by returning the
//! original near-collinear coordinates after rank selection.
...
fn reduce(rows: &[Vec<f64>]) -> Result<Reduced, InformationError> {
    ...
    for column in 0..width {
        ...
        let mut candidate = rows
            ...
            .collect::<Vec<_>>();
        let rms = (candidate.iter().map(|value| value * value).sum::<f64>() / n as f64).sqrt();
        if !rms.is_finite() {
            return Err(InformationError::NonFiniteValue);
        }
        if rms == 0.0 {
        ...
        // Two MGS passes suppress loss of orthogonality for almost-collinear
        // controls while preserving the declared input-coordinate order.
        for _ in 0..2 {
            for direction in &basis {
                let coefficient = projection(&candidate, direction);
                for (value, direction_value) in candidate.iter_mut().zip(direction) {
    ...
    for (column, direction) in basis.iter().enumerate() {
...
fn solve(mut matrix: Vec<Vec<f64>>, mut rhs: Vec<f64>) -> Result<Vec<f64>, InformationError> {
    ...
    for pivot in 0..n {
        ...
        if !matrix[best][pivot].is_finite() || matrix[best][pivot].abs() <= PIVOT_FLOOR {
...
pub fn gaussian_mi(target: &[f64], source: &[Vec<f64>]) -> Result<MiEstimate, InformationError> {
    ...
    let joint_logdet = cholesky_log2_det(&covariance(&joint_rows)?)?;
    let canonical = normalize_mi(0.5 * (var_t.log2() + source_logdet - joint_logdet))?;
...
mod tests {
    ...
    #[test]
    fn near_collinear_columns_remain_cross_method_stable() {
        let mut target = Vec::new();
        let mut source = Vec::new();
        for index in 0..128 {
            let x = index as f64 - 63.5;
            let z = ((index * 17 + 11) % 29) as f64 - 14.0;
            target.push(0.2 * x + 0.3 * z + (index as f64 / 7.0).sin());
            source.push(vec![x, x + 1.0e-8 * z, z]);
        }
        let estimate = gaussian_mi(&target, &source).unwrap();
        assert_eq!(estimate.rank(), 3);
        assert!(estimate.discrepancy_bits() <= PID_TOLERANCE_BITS);
    }
    ...
    fn source_has_no_final_holdout_authorization_secret() {
        let source = include_str!("gaussian_mmi_v7_stable.rs");
