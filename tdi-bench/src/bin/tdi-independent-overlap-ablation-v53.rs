//! TDI-5.3 operational-activation evaluator.
//!
//! This file is derived mechanically from the frozen TDI-5.2 evaluator
//! (`tdi-independent-overlap-ablation-v52.rs`), which is the sole
//! implementation source for TDI-5.3. TDI-5.2 remains frozen and
//! untouched: its scientific design (generator, seed blocks, populations,
//! budgets, exclusions, predictors, layouts, model family, bootstrap,
//! metrics and criteria) is preserved here mechanically and unchanged.
//!
//! TDI-5.2's frozen evaluator has no executable full-run entrypoint —
//! `run_full_experiment` there unconditionally returns an error. TDI-5.3
//! exists solely to provide that missing operational entrypoint, gated
//! behind an explicit, exact human confirmation environment variable (see
//! `run_full_experiment` and `tdi53_full_run_confirmed` below), without
//! ever changing a threshold, seed, constant, metric or criterion. See
//! `docs/TDI-5.3-INDEPENDENT-OVERLAP-ACTIVATION-PREREGISTRATION.md` for
//! the full governance rationale and inherited scientific design.
...
/// Runs the full TDI-5.2/TDI-5.3 pipeline (generation, per-block fitting,
...
/// `run_full_experiment`'s `--full` path, and only after that path's
/// exact confirmation-token check has passed; tests and the termination
/// smoke path never reach that branch.
...
/// Section 17, items 1-5 (as inherited via TDI-5.3 preregistration Section
/// 6): git commit, compiler/Cargo versions, and the SHA-256 of the v53
/// evaluator, the TDI-5.3 preregistration, and the TDI-5.3 scientific
/// manifest — plus the frozen TDI-5.2 ancestor's own identities, printed
/// for provenance (TDI-5.3 preregistration Section 1 requirement 5:
/// "retain provenance links to frozen TDI-5.2").
...
fn print_tdi52_criteria_conditions(report: &Tdi52ExperimentReport) {
    ...
    println!();
    println!("TDI-5.3A : {:#?}", report.criterion_a);
    println!();
    println!("TDI-5.3B : {:#?}", report.criterion_b);
    println!();
    println!("TDI-5.3C : {:#?}", report.criterion_c);
    println!();
    println!("TDI-5.3D : {:#?}", report.criterion_d);
    println!();
    println!("TDI-5.3E : {:#?}", report.criterion_e);
}
...
/// Section 17, items 18-19: the final TDI-5.3A through TDI-5.3E verdicts,
/// with TDI-5.3C's four-way classification reported on its own line.
/// These criteria are definitionally identical to TDI-5.2A through
/// TDI-5.2E (TDI-5.3 preregistration Section 4); only the printed label
/// changes.
fn print_tdi52_final_verdicts(report: &Tdi52ExperimentReport) {
    println!();
    println!("=== VERDICTS FINAUX (Section 17, items 18-19) ===");
    println!(
        "TDI-5.3A — signal joint                    : {}",
        tdi52_verdict_label(report.criterion_a.succeeded())
    );
    println!(
        "TDI-5.3B — signal O2 indépendant            : {}",
        tdi52_verdict_label(report.criterion_b.succeeded())
    );
    println!(
        "TDI-5.3C — classification O1 indépendante   : {}",
        report.criterion_c.classification.label()
    );
    println!(
        "TDI-5.3D — transfert OOD (largeurs 5 et 6)  : {}",
        tdi52_verdict_label(report.criterion_d.succeeded())
    );
    println!(
        "TDI-5.3E — trajectoire multi-horizon        : {}",
        tdi52_verdict_label(report.criterion_e.succeeded())
    );
}

/// Prints the complete required raw output checklist (all 20 items,
/// inherited unchanged from TDI-5.2 Section 17 via TDI-5.3
/// preregistration Section 6) for a completed pipeline run. Purely a
/// presentation layer over `Tdi52ExperimentReport`: it has no
/// scale-awareness of its own, so it is exercised at tiny scale by the
/// termination smoke path and by tests. It only ever prints the real
/// 165,000-record run's output when called from `run_full_experiment`'s
/// `--full` path, and only after that path's exact confirmation-token
/// check has passed.
fn print_tdi52_required_raw_output(report: &Tdi52ExperimentReport) {
    ...
    print_tdi52_aggregate_comparison(
        "TDI-5.3D — transfert OOD largeur 5",
        PRIMARY_HORIZON,
        &report.criterion_d_comparisons.0,
    );
    print_tdi52_aggregate_comparison(
        "TDI-5.3D — transfert OOD largeur 6",
        PRIMARY_HORIZON,
    ...
    for horizon_comparison in &report.criterion_e_horizon_comparisons {
        ...
        print_tdi52_aggregate_comparison(
            &format!("TDI-5.3E — trajectoire secondaire U_{horizon}"),
            horizon,
...
fn run_termination_smoke() -> Result<(), String> {
    println!("=== TDI-5.3 TERMINATION SMOKE ===");
...
/// Name of the environment variable that must carry the exact TDI-5.3
/// full-run confirmation value. See TDI-5.3 preregistration Section 5.
const TDI53_FULL_RUN_CONFIRMATION_VAR: &str = "TDI53_CONFIRM_FULL_RUN";

/// The one accepted value for `TDI53_FULL_RUN_CONFIRMATION_VAR`. Any
/// other value, or the variable being unset, must refuse `--full`.
const TDI53_FULL_RUN_CONFIRMATION_VALUE: &str = "I_ACCEPT_THE_TDI53_FREEZE_RULE";
...
/// unit tested directly without ever touching a real environment
/// variable or risking the accepted branch reaching
/// `run_full_experiment` (and, through it, the real pipeline).
fn tdi53_full_run_confirmed(value: Option<&str>) -> bool {
    value == Some(TDI53_FULL_RUN_CONFIRMATION_VALUE)
}

fn tdi53_usage_error() -> String {
    format!(
        "usage: tdi-independent-overlap-ablation-v53 --termination-smoke|--preflight|--full\n\
         ...
         {TDI53_FULL_RUN_CONFIRMATION_VAR}={TDI53_FULL_RUN_CONFIRMATION_VALUE}"
    )
}
...
fn tdi53_parse_mode(arguments: &[String]) -> Result<Tdi53Mode, String> {
    match arguments {
        [flag] if flag == "--termination-smoke" => Ok(Tdi53Mode::TerminationSmoke),
        [flag] if flag == "--preflight" => Ok(Tdi53Mode::Preflight),
        [flag] if flag == "--full" => Ok(Tdi53Mode::Full),
        _ => Err(tdi53_usage_error()),
    }
}
...
fn main() -> Result<(), String> {
    ...
    match tdi53_parse_mode(&arguments)? {
        Tdi53Mode::TerminationSmoke => run_termination_smoke(),
        Tdi53Mode::Preflight => run_preflight(),
        Tdi53Mode::Full => run_full_experiment(),
    }
}

/// The TDI-5.3 operational activation entrypoint. Unlike TDI-5.2's frozen
/// `run_full_experiment` (an unconditional guard), this checks the exact
/// confirmation environment variable *before* any generation, fitting or
/// bootstrap; only when it matches does this call the real full pipeline
/// exactly once, over the real preregistered `population_specs()`, and
/// print the complete required raw output. See TDI-5.3 preregistration
/// Section 5.
fn run_full_experiment() -> Result<(), String> {
    let confirmation = std::env::var(TDI53_FULL_RUN_CONFIRMATION_VAR).ok();

    if !tdi53_full_run_confirmed(confirmation.as_deref()) {
        return Err(format!(
            "TDI-5.3 full execution requires the exact confirmation environment \
             variable {TDI53_FULL_RUN_CONFIRMATION_VAR}={TDI53_FULL_RUN_CONFIRMATION_VALUE}; \
             refusing before any generation, fitting or bootstrap"
        ));
    }

    let report = run_tdi52_pipeline(&population_specs())?;
...
/// TDI-5.3 preflight: verifies the complete frozen configuration (seed
/// reservations, population counts, bootstrap constants, pipeline
/// wiring) and prints identities and the exact real-run command, without
/// ever generating a scientific population. See TDI-5.3 preregistration
/// Section 5.
fn run_preflight() -> Result<(), String> {
    println!();
    println!("=== TDI-5.3 PREFLIGHT (aucune génération scientifique) ===");
    ...
    for seed_block in FROZEN_BLOCK_ORDER {
        ...
        if block_total != 55_000 {
            return Err(format!(
                "block {} requests {block_total} accepted records, expected 55,000",
                seed_block.label()
            ));
        }
    }
    ...
    if grand_total != 165_000 {
        return Err(format!(
            "total requested accepted records is {grand_total}, expected 165,000"
        ));
    }
    ...
    println!(
        "graines de bootstrap par bloc                   : A=0x{:016X} B=0x{:016X} C=0x{:016X}",
        SeedBlockId::A.bootstrap_seed(),
        SeedBlockId::B.bootstrap_seed(),
        SeedBlockId::C.bootstrap_seed()
    );
    println!("graine de bootstrap agrégé stratifié            : 0x{AGGREGATE_BOOTSTRAP_SEED:016X}");
    println!(
        "pipeline complet câblé à --full                 : oui (run_tdi52_pipeline, \
         subordonné à {TDI53_FULL_RUN_CONFIRMATION_VAR})"
    );
    ...
    println!("  {TDI53_FULL_RUN_CONFIRMATION_VAR}={TDI53_FULL_RUN_CONFIRMATION_VALUE} \\");
    println!("    bash scripts/reproduce-tdi5.3.sh");
...
mod tests {
    ...
    #[test]
    fn frozen_tdi53_protocol_hashes_are_unchanged() {
        // TDI-5.3 is mechanically derived from the frozen TDI-5.2
        // evaluator (see this file's own header comment), so its own
        // frozen-protocol guard must pin TDI-5.2's files — its immediate
        // ancestor — not TDI-5.1's (the grandparent). This mirrors
        // tdi-independent-overlap-ablation-v52.rs's own
        // `frozen_tdi52_protocol_hashes_are_unchanged`, which correctly
        // pins TDI-5.1's files for the same reason one generation up.
        let expected_hashes = [
            (
                ".github/workflows/tdi52-ci.yml",
                "1d1f14b12b3988a40c1896efb6a9ca055fd7892a60783f543785daf3581c2187",
            ),
            (
                "docs/TDI-5.2-INDEPENDENT-OVERLAP-ABLATION-EVALUATOR.sha256",
                "7d007060f0d3053be233e49e75d0e9888c3d22ca4eda3ebb926da29fd295d275",
            ),
            (
            ...
            (
                ...
                "7f6bce51bdb21f028262e67386905ae6ac6e0ee0176d81dc72484db8fceec669",
            ),
            (
            ...
            (
                ...
                "ca32b0d39d7d0ce013ee378031114c64d0778b8b13d8c37df8b64f104666aa3e",
            ),
            (
    ...
    #[test]
    fn full_execution_guard_rejects_the_full_run_by_default() {
        assert_eq!(
            super::population_specs().len(),
            super::TOTAL_SEED_RESERVATIONS
        );

        // This calls the real `run_full_experiment`, but that function
        // checks the exact confirmation environment variable *before*
        // doing anything else, so as long as the ambient test/CI
        // environment does not set it (it never should, and nothing in
        // this codebase or its CI workflow ever does), this can only
        // ever reach the refusal branch -- it can never reach the real
        // pipeline.
        let error = super::run_full_experiment().expect_err(
            "full TDI-5.3 execution must be refused without the exact confirmation token",
        );

        assert!(
            error.contains(super::TDI53_FULL_RUN_CONFIRMATION_VAR),
            "unexpected error message: {error}"
        );
        assert!(
            error.contains(super::TDI53_FULL_RUN_CONFIRMATION_VALUE),
            "unexpected error message: {error}"
        );
    }

    #[test]
    fn tdi53_full_run_confirmation_validator_accepts_only_the_exact_value() {
        assert!(!super::tdi53_full_run_confirmed(None));
        assert!(!super::tdi53_full_run_confirmed(Some("")));
        assert!(!super::tdi53_full_run_confirmed(Some(
        ...
        assert!(!super::tdi53_full_run_confirmed(Some(
            "I_ACCEPT_THE_TDI52_FREEZE_RULE"
        )));
        assert!(super::tdi53_full_run_confirmed(Some(
            super::TDI53_FULL_RUN_CONFIRMATION_VALUE
        )));
    }
    ...
    // the pipeline function that `run_full_experiment`'s `--full` path
    // now calls with the real specs (behind its confirmation check).
    #[test]
    ...
    #[test]
    fn tdi53_usage_error_mentions_every_valid_flag_and_the_confirmation_variable() {
        let message = super::tdi53_usage_error();

        for expected_fragment in [
            "--termination-smoke",
            "--preflight",
            "--full",
            super::TDI53_FULL_RUN_CONFIRMATION_VAR,
            super::TDI53_FULL_RUN_CONFIRMATION_VALUE,
        ] {
            assert!(
                message.contains(expected_fragment),
                "usage message {message:?} missing {expected_fragment:?}"
            );
        }
    }

    #[test]
    fn tdi53_parse_mode_rejects_a_bare_no_argument_invocation() {
        let error = super::tdi53_parse_mode(&[])
            .expect_err("a bare invocation must not select any mode, especially not --full");

        assert!(error.contains("--termination-smoke"));
        assert!(error.contains("--preflight"));
        assert!(error.contains("--full"));
    }

    #[test]
    fn tdi53_parse_mode_selects_full_only_for_the_exact_single_flag() {
        assert_eq!(
            super::tdi53_parse_mode(&["--full".to_owned()]),
            Ok(super::Tdi53Mode::Full)
        );
        assert_eq!(
            super::tdi53_parse_mode(&["--preflight".to_owned()]),
            Ok(super::Tdi53Mode::Preflight)
        );
        assert_eq!(
            super::tdi53_parse_mode(&["--termination-smoke".to_owned()]),
            Ok(super::Tdi53Mode::TerminationSmoke)
        );
        assert!(super::tdi53_parse_mode(&[]).is_err());
        assert!(super::tdi53_parse_mode(&["--fullx".to_owned()]).is_err());
        assert!(super::tdi53_parse_mode(&["--full".to_owned(), "extra".to_owned()]).is_err());
    }

    #[test]
    fn run_preflight_performs_no_scientific_generation() {
        let start = std::time::Instant::now();
        super::run_preflight().expect("preflight must succeed without any scientific generation");
        let elapsed = start.elapsed();

        assert!(
            elapsed < std::time::Duration::from_secs(5),
            "preflight took {elapsed:?}; a real generation call would take far longer \
             (width 6 alone costs roughly 11s per accepted record)"
        );
    }

    fn tdi53_read_repo_file(relative_path: &str) -> String {
        std::fs::read_to_string(super::tdi52_repository_root().join(relative_path))
            .unwrap_or_else(|error| panic!("failed to read {relative_path}: {error}"))
    }

    #[test]
    fn run_termination_smoke_uses_bounded_tiny_specs_not_the_real_population_specs() {
        // Calling `run_termination_smoke` for real costs roughly as much
        // as `run_tdi52_pipeline_succeeds_end_to_end_at_tiny_scale` (it
        // exercises the same tiny-scale pipeline), so boundedness is
        // checked here by source inspection instead of paying that cost
        // a second time: the function must construct its own tiny
        // (`target_count: 1`) specs, never the real, 165,000-record
        // `population_specs()` output.
        let source =
            tdi53_read_repo_file("tdi-bench/src/bin/tdi-independent-overlap-ablation-v53.rs");
        ...
        let end = source[start..]
            ...
            .expect("main must follow run_termination_smoke");
        let body = &source[start..end];

        assert!(
            body.contains("target_count: 1"),
            "run_termination_smoke must only ever use tiny, bounded specs"
        );
        assert!(
            !body.contains("run_tdi52_pipeline(&population_specs())"),
            "run_termination_smoke must never call the pipeline with the real specs"
        );
    }

    #[test]
    fn run_full_experiment_is_wired_to_the_real_pipeline_on_the_accepted_path() {
        // Verified by source inspection, not by executing the accepted
        // path: this must never actually reach the real pipeline during
        // a test run.
        let source =
            tdi53_read_repo_file("tdi-bench/src/bin/tdi-independent-overlap-ablation-v53.rs");
        ...
        assert!(
            body.contains("run_tdi52_pipeline(&population_specs())"),
            "run_full_experiment's accepted path must call the real pipeline over the \
             real preregistered population_specs()"
        );
        assert!(
            body.contains("print_tdi52_required_raw_output"),
            "run_full_experiment must print the complete required raw output on success"
        );
        assert!(
            body.contains("tdi53_full_run_confirmed"),
            "run_full_experiment must gate the pipeline call behind the pure validator"
        );
    }

    #[test]
    fn reproduction_script_invokes_full_and_requires_the_exact_token() {
        let script = tdi53_read_repo_file("scripts/reproduce-tdi5.3.sh");
        ...
        assert!(script.contains(super::TDI53_FULL_RUN_CONFIRMATION_VAR));
        assert!(script.contains(super::TDI53_FULL_RUN_CONFIRMATION_VALUE));
        assert!(script.contains("require_full_run_confirmation"));
    }

    #[test]
    fn reproduction_script_refuses_to_overwrite_an_existing_result() {
        let script = tdi53_read_repo_file("scripts/reproduce-tdi5.3.sh");
    ...
    #[test]
    fn tdi52_frozen_files_are_covered_by_the_ancestor_hash_test() {
        // A thin, explicit cross-check that the ancestor-hash test above
        // (`frozen_tdi53_protocol_hashes_are_unchanged`) really does pin
        // every file TDI-5.3's own preregistration Section 1 lists as
        // "must remain byte-for-byte unchanged".
        let preregistration =
            tdi53_read_repo_file("docs/TDI-5.3-INDEPENDENT-OVERLAP-ACTIVATION-PREREGISTRATION.md");
        ...
        for frozen_path in [
            ...
            assert!(
                preregistration.contains(frozen_path),
                "TDI-5.3 preregistration Section 1 must list {frozen_path} as frozen"
            );
        }
    }
