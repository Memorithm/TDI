//! TDI-5.9 spectral-moment-saturation evaluator (fourth exact spectral
//! moment; descriptor saturation).
//!
//! This file is derived from the frozen TDI-5.6 evaluator
//! (`tdi-independent-overlap-ablation-v56.rs`). It is **not** a continuation
//! of TDI-5.7 or TDI-5.8 (separate, unrelated derivations of TDI-5.6) or of
//! any TDI-6.x experiment; its single scientific ancestor is TDI-5.6
//! (preregistration Section 1, Section 3). TDI-5.9 reuses TDI-5.6's frozen
...
//! second and third exact spectral moments (s2, s3), ridge machinery,
//! deterministic bootstrap engine and the four-way
//! Beneficial/Equivalent/Harmful/Inconclusive classification logic without
//! altering any of them.
//!
//! TDI-5.9 makes exactly the scientific additions its preregistration
//! (`docs/TDI-5.9-SPECTRAL-MOMENT-SATURATION-PREREGISTRATION.md`) declares
//! and nothing else:
//!
//!   * one more *exact* spectral moment of the one-step Noop kernel — the
//!     fourth moment `s4 = trace(P^4)`, extending the inherited `s2, s3`
//!     closed-walk computation with one more nested loop level (a closed
//!     4-walk `i -> j -> k -> l -> i` contributes the unit fraction
//!     `1/(d_i d_j d_k d_l)`), accumulated with the same arbitrary-precision
//!     `ExactRatio` addition and rounded once — and two new linear layouts,
//!     `SK4` (baseline + delta + delta_bar + s2 + s3 + s4) and `SKT4` (SK4 +
//!     O1 + O2). The inherited `CK`, `SK` and `SKT` layouts (delta, delta_bar
//!     and, for SK/SKT, only s2/s3) are kept unchanged: `SK` minus `CK`,
//!     recomputed fresh on TDI-5.9's own data, is one half of the criterion
//!     TDI-5.9D saturation comparison;
//!   * three fresh, independent seed blocks Y/Z/AA, disjoint from every
//!     earlier block (TDI-5.2 through TDI-6.4), with fresh bootstrap seeds;
...
//!     unchanged since TDI-5.6);
//!   * criterion TDI-5.9A (SKT4 vs SK4 at the focal horizons U3 and U6, the
//!     direct structural analog of TDI-5.6A under the richer spectral
//!     control), criterion TDI-5.9B (SK4 vs SK at U3 and U6, the fourth
//!     moment's own marginal value beyond `{delta, delta_bar, s2, s3}` —
//!     the structural analog of TDI-5.6B), criterion TDI-5.9C (the
//!     SKT4-vs-SK4 relative-MSE reduction across the dense grid plus a
//!     decay-law / redundancy-horizon summary — the structural analog of
//!     TDI-5.6C), and criterion TDI-5.9D (descriptor saturation: the fresh
//!     SK-vs-CK reduction side by side with the SK4-vs-SK reduction from
//!     5.9B, a `saturating` boolean and a bootstrap-interval-overlap flag —
//!     purely descriptive, never classified through the four-way logic).
//!
//! The full run is gated behind an explicit, exact human confirmation
//! environment variable (see `run_full_experiment` and
//! `tdi59_full_run_confirmed`). No commit, test or CI run supplies that
//! token.
...
// The two focal horizons at which TDI-5.9A/5.9B classify (and TDI-5.9D
// reports descriptively): U3 (near, where TDI-5.4B found a short-horizon
// benefit) and the primary U6.
...
// exact cardinality/budget machinery, but TDI-5.9 generates no populations
...
// Exact spectral moments of the one-step Noop kernel, extended by TDI-5.9
// Section 5 from two closed-walk lengths to three: s2 = trace(P^2),
// s3 = trace(P^3) (both inherited unchanged from TDI-5.6 Section 5) and the
// new s4 = trace(P^4). All three are computed per candidate system as
// closed-walk sums of unit fractions with a single final rounding.
const SPECTRAL_FEATURE_COUNT: usize = 3;
// The inherited SK/SKT layouts (TDI-5.6 Section 6) use only the first two
// entries of `Record::spectral` (s2, s3); this legacy count anchors their
// feature-count formulas so SK/SKT stay at their frozen 17/19 features even
// though `SPECTRAL_FEATURE_COUNT` has grown to 3 for the new SK4/SKT4
// layouts (Section 6).
const SK_SPECTRAL_FEATURE_COUNT: usize = 2;

// Linear layouts, inherited from TDI-5.2/5.3/5.4/5.5. In TDI-5.9 (as in
// TDI-5.6) they are exploratory only (Section 6) and determine no
...
// Confirmatory-lineage linear layouts (Section 6). CK (inherited from
// TDI-5.5) adds the two exact contraction descriptors to the baseline; SK
// additionally adds the two second/third exact spectral moments; SKT
// additionally adds the two early overlaps. CK, SK and SKT are inherited
// unchanged from TDI-5.6 -- kept only as recomputation/reference layouts, no
// longer confirmatory in their own right (Section 3, Section 6): SK minus CK
// (recomputed fresh on TDI-5.9's own data) is one half of the TDI-5.9D
// saturation comparison. SK4 additionally adds the new fourth exact spectral
// moment; SKT4 additionally adds the two early overlaps. SK4 minus SK
// isolates the fourth moment's marginal value beyond `{delta, delta_bar, s2,
// s3}` (criterion 5.9B); SKT4 minus SK4 isolates the overlaps' marginal value
// *after* the contraction descriptors and all three exact spectral moments
// are present (criteria 5.9A, 5.9C).
...
//   SK4  = baseline + delta + delta_bar + s2 + s3 + s4               (13 + 5 = 18)
//   SKT4 = baseline + delta + delta_bar + s2 + s3 + s4 + O1 + O2     (13 + 7 = 20)
const CK_FEATURE_COUNT: usize = BASELINE_FEATURE_COUNT + CONTRACTION_FEATURE_COUNT;
const SK_FEATURE_COUNT: usize =
    BASELINE_FEATURE_COUNT + CONTRACTION_FEATURE_COUNT + SK_SPECTRAL_FEATURE_COUNT;
const SKT_FEATURE_COUNT: usize = BASELINE_FEATURE_COUNT
    + CONTRACTION_FEATURE_COUNT
    + SK_SPECTRAL_FEATURE_COUNT
    + EARLY_OVERLAP_FEATURE_COUNT;
const SK4_FEATURE_COUNT: usize =
    BASELINE_FEATURE_COUNT + CONTRACTION_FEATURE_COUNT + SPECTRAL_FEATURE_COUNT;
const SKT4_FEATURE_COUNT: usize = SK4_FEATURE_COUNT + EARLY_OVERLAP_FEATURE_COUNT;

const MODEL_LAYOUT_COUNT: usize = 10;

const RIDGE_LAMBDA: f64 = 1.0;
const BOOTSTRAP_REPLICATES: usize = 4_000;
// Fresh stratified-aggregate bootstrap seed (TDI-5.9 Section 10), disjoint
// from every earlier bootstrap seed (TDI-5.2 through TDI-6.4), following the
// `TDI5` ASCII-prefix scheme (`0x5444_4935` = "TDI5") established from
// TDI-5.7 onward, with the ASCII digit "9" (`0x39`).
const AGGREGATE_BOOTSTRAP_SEED: u64 = 0x5444_4935_3900_4700;
...
enum SeedBlockId {
    Y,
    Z,
    // The single-capital-letter block-naming sequence begun at TDI-5.2 (A, B,
    // C, ...) has, by TDI-6.4, used every letter through X; TDI-5.9's first
    // two blocks take the sequence's last two letters, Y and Z, and its third
    // continues with the spreadsheet-style extension AA rather than reusing
    // any earlier letter (Section 9).
    Aa,
}
...
// Fresh seed blocks Y/Z/AA, disjoint from every prior block -- TDI-6.4, the
// most recently allocated block, consumes seeds up to ~9.23e9; TDI-5.9 starts
// at 1.0e10 (Section 9). New bootstrap seeds, disjoint from every earlier
// bootstrap seed (Section 10), following the `TDI5` ASCII-prefix scheme
// (`0x5444_4935` = "TDI5") established from TDI-5.7 onward, with the ASCII
// digit "9" (`0x39`).
const SEED_BLOCKS: [SeedBlockSpec; SEED_BLOCK_COUNT] = [
    SeedBlockSpec {
        id: SeedBlockId::Y,
        training_width_3_seed: 10_000_000_000,
        holdout_width_3_seed: 10_010_000_000,
        training_width_4_seed: 10_020_000_000,
        holdout_width_4_seed: 10_030_000_000,
        bootstrap_seed: 0x5444_4935_3900_0001,
    },
    SeedBlockSpec {
        id: SeedBlockId::Z,
        training_width_3_seed: 10_100_000_000,
        holdout_width_3_seed: 10_110_000_000,
        training_width_4_seed: 10_120_000_000,
        holdout_width_4_seed: 10_130_000_000,
        bootstrap_seed: 0x5444_4935_3900_0002,
    },
    SeedBlockSpec {
        id: SeedBlockId::Aa,
        training_width_3_seed: 10_200_000_000,
        holdout_width_3_seed: 10_210_000_000,
        training_width_4_seed: 10_220_000_000,
        holdout_width_4_seed: 10_230_000_000,
        bootstrap_seed: 0x5444_4935_3900_0003,
    },
];
...
#[repr(usize)]
enum FeatureLayout {
    // Linear layouts B0..BD are exploratory (inherited from TDI-5.2 through
    // TDI-5.6). Their discriminants (0..4) are preserved so `layout as usize`
    // indexing is unchanged from TDI-5.2/5.3/5.4/5.5/5.6. CK/SK/SKT (also
    // inherited from TDI-5.5/5.6, unchanged) follow, kept only as
    // recomputation/reference layouts (Section 3, Section 6); the new
    // confirmatory layouts SK4/SKT4 follow them.
    B0,
    B1,
    B2,
    B12,
...
impl FeatureLayout {
    ...
    const fn label(self) -> &'static str {
        match self {
            ...
            Self::Sk => "SK — BASELINE + δ + δ̄ + s2 + s3 (contraction + spectral s2/s3)",
            Self::Skt => {
                "SKT — BASELINE + δ + δ̄ + s2 + s3 + O1 + O2 (contraction + spectral s2/s3 + TDI)"
            }
            Self::Sk4 => "SK4 — BASELINE + δ + δ̄ + s2 + s3 + s4 (full exact spectral baseline)",
            Self::Skt4 => "SKT4 — BASELINE + δ + δ̄ + s2 + s3 + s4 + O1 + O2 (full model)",
        }
    }
    ...
    const fn feature_count(self) -> usize {
        match self {
            ...
            Self::Sk4 => SK4_FEATURE_COUNT,
            Self::Skt4 => SKT4_FEATURE_COUNT,
        }
    }
}
...
/// Section 5, inherited unchanged per TDI-5.9 Section 3 -- TDI-5.6 Section 5
/// is the *spectral moments*, not these): the Dobrushin coefficient
/// `delta = max_{i<j} TV(P_i, P_j)`
...
/// Exact spectral moments of the one-step Noop kernel. `s2 = trace(P^2)` and
/// `s3 = trace(P^3)` are inherited unchanged from TDI-5.6 Section 5; TDI-5.9
/// Section 5 extends the computation with one more closed-walk length, the
/// fourth moment `s4 = trace(P^4)`. Each `P(s, .)` is the exact uniform
/// distribution over state `s`'s `d_s` Noop successors
...
///   s4 = sum over ordered quadruples (i, j, k, l) with j a successor of i,
...
/// four branching factors (each `<= 2^width`), so it fits in `u128`. The
...
/// involved; all three moments are exact rationals in `[0, 2^width]`.
fn spectral_moments(
    context: AttemptContext,
    system: &TableSystem,
    ...
    let mut fourth_moment = second_moment.clone();
    ...
    for (from_state, from_row) in &state_rows {
        ...
        for middle_state in from_row.keys() {
            ...
            for last_state in middle_row.keys() {
                ...
                // Closed 4-walk i -> j -> k -> l -> i contributes
                // 1 / (d_i d_j d_k d_l). TDI-5.9's one added nested loop
                // level beyond the inherited TDI-5.6 second/third-moment
                // computation above.
                for fourth_state in last_row.keys() {
                    let fourth_row = resolve(fourth_state)?;
                    let fourth_degree = fourth_row.len() as u128;

                    if fourth_row.contains_key(from_state) {
                        let denominator = from_degree
                            .checked_mul(middle_degree)
                            .and_then(|partial| partial.checked_mul(last_degree))
                            .and_then(|partial| partial.checked_mul(fourth_degree))
                            .ok_or_else(|| arithmetic("spectral 4-walk denominator overflowed"))?;

                        let term = ExactRatio::new(1, denominator)
                            .ok_or_else(|| arithmetic("spectral 4-walk term is invalid"))?;

                        fourth_moment = fourth_moment
                            .checked_add(&term)
                            .ok_or_else(|| arithmetic("spectral fourth-moment sum overflowed"))?;
                    }
                }
            }
        }
    ...
    Ok([
        second_moment.as_f64(),
        third_moment.as_f64(),
        fourth_moment.as_f64(),
    ])
}
...
fn feature_layout(record: &Record, layout: FeatureLayout) -> Vec<f64> {
    ...
    match layout {
        ...
        // Confirmatory-lineage layouts (Section 6). Terms are the two exact
        // contraction descriptors (delta, delta_bar) and, for SK/SKT, the
        // first two exact spectral moments (s2, s3) only -- both inherited
        // unchanged from TDI-5.6, all already stored on the record;
        ...
        // isolates the second/third moments' marginal value beyond
        // contraction and SKT minus SK isolates the overlaps' marginal value
        // beyond both (both recomputed fresh on TDI-5.9's own data; no longer
        // confirmatory in their own right, Section 3).
        FeatureLayout::Ck => {
        ...
        // New TDI-5.9 confirmatory layouts (Section 6). SK4 additionally adds
        // the new third stored spectral moment (s4); SKT4 additionally adds
        // the two early overlaps. SK4 minus SK isolates s4's own marginal
        // value beyond `{delta, delta_bar, s2, s3}` (criterion 5.9B); SKT4
        // minus SK4 isolates the overlaps' marginal value beyond the
        // contraction descriptors and all three exact spectral moments
        // (criteria 5.9A, 5.9C).
        FeatureLayout::Sk4 => {
            features.push(record.contraction[0]);
            features.push(record.contraction[1]);
            features.push(record.spectral[0]);
            features.push(record.spectral[1]);
            features.push(record.spectral[2]);
        }
        FeatureLayout::Skt4 => {
            features.push(record.contraction[0]);
            features.push(record.contraction[1]);
    ...
    let (attempt_multiplier, no_progress_limit) = match width {
        ...
        _ => {
            return Err(EvaluationError::new(
                context,
                FailureCategory::UnsupportedWidth,
                format!("width {width} is not part of the TDI-5.9 preregistered populations"),
            ));
        }
    };
...
impl BlockPopulations {
    ...
    /// named fields directly. TDI-5.9 has no OOD populations (Section 8).
...
const FROZEN_BLOCK_ORDER: [SeedBlockId; SEED_BLOCK_COUNT] =
    [SeedBlockId::Y, SeedBlockId::Z, SeedBlockId::Aa];
...
fn validate_frozen_block_order(seed_blocks: &[SeedBlockId]) -> Result<(), String> {
    ...
    for (&actual, &expected) in seed_blocks.iter().zip(&FROZEN_BLOCK_ORDER) {
        if actual != expected {
            return Err(format!(
                "requires deterministic block order Y, Z, AA; found {} where {} was expected",
                actual.label(),
                expected.label()
            ));
        }
...
/// reconstructed-O metrics and its prediction set. TDI-5.9, like TDI-5.6,
/// always compares two fitted layouts, so this carries no layout identity of
/// its own.
#[derive(Clone, Debug)]
...
/// reconstructed-O metrics plus its prediction set. TDI-5.9 compares two
/// fitted layouts only — the naive persistence competitor of TDI-5.5 stays
/// dropped (preregistration Section 7, unchanged since TDI-5.6), so every
/// comparison runs through the identical paired / stratified-aggregate
...
/// Criterion TDI-5.9A (Section 13): the SKT4-vs-SK4 four-way classification
/// at the focal horizons U3 and U6 -- the direct structural analog of
/// TDI-5.6A under the richer three-moment spectral control. SKT4 minus SK4
/// isolates the overlaps' marginal value *after* the exact contraction
/// descriptors and all three exact spectral moments are present.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct Tdi59CriterionA {
    focal_classifications: [CriterionCClassification; FOCAL_HORIZON_COUNT],
}

/// Criterion TDI-5.9B (Section 14): the SK4-vs-SK four-way classification at
/// the focal horizons U3 and U6 -- the direct structural analog of TDI-5.6B,
/// now comparing SK4 to SK instead of SK to CK. SK4 minus SK isolates the
/// fourth exact spectral moment's own marginal value beyond
/// `{delta, delta_bar, s2, s3}`.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct Tdi59CriterionB {
    focal_classifications: [CriterionCClassification; FOCAL_HORIZON_COUNT],
}

/// Criterion TDI-5.9C decay-law / redundancy-horizon summary (Section 15) --
/// the direct structural analog of TDI-5.6C. Descriptive only: it reports the
/// overlaps' marginal value beyond the full exact descriptor set (contraction
/// and all three spectral moments) across the dense grid, whether that value
/// is non-increasing in horizon, the redundancy horizon h★ (the first horizon
/// classified Equivalent), and the successive ratios for inspecting a
/// geometric shape.
#[derive(Clone, Debug, PartialEq)]
struct Tdi59CriterionC {
    horizons: Vec<usize>,
...
/// Pure core of the TDI-5.9C decay-law summary, over bare per-horizon
...
/// True iff two 95% confidence intervals overlap. Used by criterion TDI-5.9D
/// to operationalize Section 16's descriptive "never read as a confident
/// inequality when the two intervals are indistinguishable from noise"
/// language against quantities already computed (the two aggregate bootstrap
/// intervals), without inventing any new numeric threshold. Mirrors the
/// identically-named, identically-defined helper introduced by TDI-6.4.
...
/// One focal horizon's entry of criterion TDI-5.9D (Section 16): the
/// descriptor-saturation comparison. Purely descriptive -- carries no
/// `CriterionCClassification` field, so it can never be run through the
/// four-way classifier or reported as a pass/fail verdict (Section 4.4,
/// Section 16).
#[derive(Clone, Copy, Debug, PartialEq)]
struct Tdi59SaturationEntry {
    horizon: usize,
    // (a) The aggregate relative-MSE reduction of SK over CK: the combined
    // marginal value of `{s2, s3}`, a fresh replicate of TDI-5.6B's
    // comparison computed on TDI-5.9's own Y/Z/AA holdout (Section 4.4).
    spectral_pair_reduction: f64,
    spectral_pair_interval: ConfidenceInterval,
    // (b) The aggregate relative-MSE reduction of SK4 over SK: the fourth
    // moment's own marginal value -- criterion TDI-5.9B's own point estimate.
    fourth_moment_reduction: f64,
    fourth_moment_interval: ConfidenceInterval,
    // (2) < (1): whether the single fourth moment's marginal reduction is
    // smaller than the combined second-and-third-moment reduction.
    saturating: bool,
    // Whether the two reductions' 95% bootstrap intervals overlap, so
...
/// Criterion TDI-5.9D descriptor-saturation summary (Section 16): one
/// `Tdi59SaturationEntry` per focal horizon (U3, U6). Genuinely new
/// structure with no 1:1 analog in TDI-5.6 (Section 4.4): it makes **no**
/// success/failure claim and does not gate any other criterion.
#[derive(Clone, Debug, PartialEq)]
struct Tdi59CriterionD {
    entries: Vec<Tdi59SaturationEntry>,
}

#[derive(Clone, Debug)]
struct Tdi59ExperimentReport {
    blocks: Vec<BlockPopulations>,
    aggregate_fit: AggregateModelFit,
    // TDI-5.9A + 5.9C: SKT4 vs SK4 across the dense horizon grid U3..U8.
    spectral_grid: Vec<HorizonComparison>,
    criterion_a: Tdi59CriterionA,
    criterion_c: Tdi59CriterionC,
    // TDI-5.9B: SK4 vs SK at the focal horizons (the fourth moment's own
    // marginal value). Also feeds criterion TDI-5.9D's component (b).
    fourth_moment_focal: Vec<HorizonComparison>,
    criterion_b: Tdi59CriterionB,
    // TDI-5.9D component (a): SK vs CK at the focal horizons -- a fresh
    // replicate of TDI-5.6B's comparison on TDI-5.9's own data (Section 4.4).
    // Its `CriterionCResult` classification is computed (every
    // `HorizonComparison` carries one) but is deliberately never surfaced as
    // a TDI-5.9D verdict: TDI-5.9D is purely descriptive (Section 16).
    spectral_pair_focal: Vec<HorizonComparison>,
    criterion_d: Tdi59CriterionD,
}

/// Runs the full TDI-5.9 pipeline (generation of the width-3/width-4
/// populations across seed blocks Y/Z/AA, per-block ridge fitting on the
/// contraction- and full-three-moment-spectral-inclusive design, aggregation,
/// and the four TDI-5.9 criteria) over an arbitrary set of population
/// specifications. Callers control scale entirely through `population_specs`:
/// the preregistered `population_specs()` output requests the real
/// 120,000-record run, while tests and the termination smoke path pass tiny
/// synthetic-scale specs instead. This function is called with the real specs
/// only from `run_full_experiment`'s `--full` path, and only after that
/// path's exact confirmation-token check has passed; tests and the
/// termination smoke path never reach that branch.
fn run_tdi59_pipeline(
    population_specs: &[PopulationSpec],
) -> Result<Tdi59ExperimentReport, String> {
    ...
    // TDI-5.9A + 5.9C: SKT4 (challenger) vs SK4 (baseline) across the dense
    // horizon grid; the overlaps' marginal value beyond contraction and all
    // three exact spectral moments.
    ...
    let criterion_a = Tdi59CriterionA {
    ...
    let criterion_c = Tdi59CriterionC {
        horizons,
        relative_reductions,
        classifications,
        monotone_non_increasing,
    ...
    // TDI-5.9B: SK4 (challenger) vs SK (baseline) at the focal horizons U3
    // and U6; the fourth exact spectral moment's own marginal value beyond
    // `{delta, delta_bar, s2, s3}`.
    let mut fourth_moment_focal = Vec::with_capacity(FOCAL_HORIZON_COUNT);
    for &horizon_index in &focal_indices {
        fourth_moment_focal.push(evaluate_horizon_comparison(
            horizon_index,
            &aggregate_fit,
            combined_holdout_refs,
            FeatureLayout::Sk,
    ...
    let criterion_b = Tdi59CriterionB {
        focal_classifications: std::array::from_fn(|slot| {
            fourth_moment_focal[slot].result.classification
        }),
    };

    // TDI-5.9D component (a): SK (challenger) vs CK (baseline) at the focal
    // horizons U3 and U6 -- a fresh replicate of TDI-5.6B's comparison
    // (Section 4.4), computed on TDI-5.9's own Y/Z/AA holdout under the
    // identical deterministic bootstrap. Purely descriptive: its
    // classification is computed (every `HorizonComparison` carries one) but
    // is never surfaced below as a TDI-5.9D verdict.
    let mut spectral_pair_focal = Vec::with_capacity(FOCAL_HORIZON_COUNT);
    for &horizon_index in &focal_indices {
        spectral_pair_focal.push(evaluate_horizon_comparison(
            horizon_index,
            &aggregate_fit,
            combined_holdout_refs,
            FeatureLayout::Ck,
    ...
    let criterion_d = Tdi59CriterionD {
        entries: (0..FOCAL_HORIZON_COUNT)
            .map(|slot| {
                let spectral_pair = &spectral_pair_focal[slot];
                let fourth_moment = &fourth_moment_focal[slot];

                let spectral_pair_interval = spectral_pair
                    .comparison
                    .aggregate_bootstrap
                    .relative_standardized_mse;
                let fourth_moment_interval = fourth_moment
                    .comparison
                    .aggregate_bootstrap
                    .relative_standardized_mse;

                Tdi59SaturationEntry {
                    horizon: FOCAL_HORIZONS[slot],
                    spectral_pair_reduction: spectral_pair.aggregate_relative_reduction,
                    spectral_pair_interval,
                    fourth_moment_reduction: fourth_moment.aggregate_relative_reduction,
                    fourth_moment_interval,
                    saturating: fourth_moment.aggregate_relative_reduction
                        < spectral_pair.aggregate_relative_reduction,
                    intervals_overlap: confidence_intervals_overlap(
                        spectral_pair_interval,
                        fourth_moment_interval,
                    ),
                }
...
/// TDI-6.3, TDI-6.5, TDI-6.4 (preregistration Sections 1, 3, 18, 20, 22;
/// thirteen entries, in chronological build order -- TDI-6.5 was designed and
/// merged before TDI-6.4 despite the numbering, exactly as TDI-6.4's own
/// preregistration Section 1 describes). Each entry is an (identifier,
/// evaluator path, evaluator SHA-256, preregistration path, preregistration
/// SHA-256) tuple, mirroring TDI-6.4's own `FROZEN_ANCESTOR_CHAIN` (itself
/// twelve entries, one shorter, since it does not yet include itself).
/// TDI-5.9 prints this chain for provenance and, unlike every prior
/// evaluator, also verifies it as a hard runtime gate in `--preflight` and
/// `--full` (`verify_frozen_ancestor_chain`) in addition to the bounded
/// `frozen_ancestor_hashes_are_unchanged` test -- a frozen ancestor changing
/// would be a freeze violation. TDI-5.6 is TDI-5.9's direct scientific
/// ancestor (v56.rs's generator, exact candidate analysis, contraction
/// descriptors, ridge machinery, bootstrap engine and classifier are reused
/// unchanged, Section 3); every other entry is a frozen predecessor whose
/// integrity TDI-5.9 -- built after all of them -- still attests to. Hashes
/// were computed with `sha256sum` against the actual committed files in this
/// repository (not guessed) before being pinned here and in the
/// `frozen_ancestor_hashes_are_unchanged` test.
const FROZEN_ANCESTOR_CHAIN: [(&str, &str, &str, &str, &str); 13] = [
    (
        "TDI-5.1",
    ...
    (
        "TDI-6.4",
        "tdi-bench/src/bin/tdi-independent-overlap-ablation-v64.rs",
        "1c569c3d46eb7ef766f084c5051d3d437290e5357aa523305a61b5d63c9f51b5",
        "docs/TDI-6.4-CAUSAL-PROBE-PREREGISTRATION.md",
        "8933a389586ee22af9e1cddf7ed9d30d6205b7d10682fbde362a9a01e680333b",
    ),
];

/// Hard runtime gate over `FROZEN_ANCESTOR_CHAIN`: recomputes every entry's
/// evaluator and preregistration SHA-256 against the files actually on disk
/// and fails loudly on the first mismatch. Unlike `print_tdi52_provenance`
/// (which only reads and prints these hashes for provenance, the inherited
/// TDI-5.6/TDI-6.4 behavior), `--preflight` and `--full` both call this and
/// refuse to proceed if any frozen ancestor has changed -- new in TDI-5.9,
/// mirroring what the bounded `frozen_ancestor_hashes_are_unchanged` test
/// already checks at test time (Section 3, Section 23 freeze rule).
fn verify_frozen_ancestor_chain() -> Result<(), String> {
    for entry in FROZEN_ANCESTOR_CHAIN {
        verify_ancestor_entry(entry)?;
    }

    Ok(())
}

/// Verifies one `FROZEN_ANCESTOR_CHAIN` entry's evaluator and preregistration
/// hash against disk. Factored out of `verify_frozen_ancestor_chain` so both
/// the pass and the fail path are directly unit-testable against a
/// hand-built tuple, without waiting for (or tampering with) a real frozen
/// file.
fn verify_ancestor_entry(entry: (&str, &str, &str, &str, &str)) -> Result<(), String> {
    let (label, evaluator, evaluator_hash, prereg, prereg_hash) = entry;

    let got_evaluator = tdi52_sha256_of_repo_file(evaluator);
    if got_evaluator != evaluator_hash {
        return Err(format!(
            "frozen ancestor evaluator changed: {label} ({evaluator}): expected {evaluator_hash}, found {got_evaluator}"
        ));
    }

    let got_prereg = tdi52_sha256_of_repo_file(prereg);
    if got_prereg != prereg_hash {
        return Err(format!(
            "frozen ancestor preregistration changed: {label} ({prereg}): expected {prereg_hash}, found {got_prereg}"
        ));
    }
...
/// Provenance and integrity (TDI-5.9 preregistration Section 18, items
/// 1-5): git commit, compiler/Cargo versions, and the SHA-256 of the v59
/// evaluator, the TDI-5.9 preregistration and the TDI-5.9 scientific
/// manifest — plus the full frozen ancestor chain (`FROZEN_ANCESTOR_CHAIN`),
/// read live and printed for provenance (Section 1).
fn print_tdi52_provenance() {
    println!();
    println!("=== PROVENANCE ET INTÉGRITÉ (Section 18) ===");
    println!(
        "git commit                     : {}",
    ...
    println!(
        "évaluateur TDI-5.9 SHA-256      : {}",
        tdi52_sha256_of_repo_file("tdi-bench/src/bin/tdi-independent-overlap-ablation-v59.rs")
    );
    println!(
        "préenregistrement TDI-5.9 SHA-256 : {}",
        tdi52_sha256_of_repo_file("docs/TDI-5.9-SPECTRAL-MOMENT-SATURATION-PREREGISTRATION.md")
    );
    println!(
        "manifeste scientifique TDI-5.9 SHA-256 : {}",
        tdi52_sha256_of_repo_file("docs/TDI-5.9-SCIENTIFIC-CODE.sha256")
    );
...
/// Section 18, item 6: all frozen scientific constants.
fn print_tdi52_frozen_constants() {
    println!();
    println!("=== CONSTANTES GELÉES (Section 18, item 6) ===");
    ...
    println!("nombre de features spectrales (s2, s3, s4) : {SPECTRAL_FEATURE_COUNT}");
    ...
    println!("features SK4 (CK + s2 + s3 + s4)          : {SK4_FEATURE_COUNT}");
    println!("features SKT4 (SK4 + O1 + O2)             : {SKT4_FEATURE_COUNT}");
...
/// Section 18, item 7: every seed-block definition (all seeds plus each
...
fn print_tdi52_seed_block_definitions() {
    println!();
    println!("=== BLOCS DE GRAINES (Section 18, item 7) ===");
...
/// Section 18, items 8-11 and 21: requested/accepted/rejected/attempted
...
/// Section 18, items 14-15: every metric and every bootstrap interval
...
fn print_tdi52_aggregate_comparison(label: &str, horizon: usize, comparison: &AggregateComparison) {
    println!();
    println!("=== {label} — métriques et intervalles bootstrap (Section 18, items 14-15) ===");
...
/// TDI-5.9 Section 18: every block-level and aggregate-level sub-condition of
/// the per-horizon SKT4-vs-SK4, focal SK4-vs-SK and focal SK-vs-CK
/// comparisons and the four criterion summaries, printed via `Debug` so the
/// output can never silently drift from the named fields it reflects.
/// Criterion TDI-5.9D carries no `CriterionCClassification` of its own (its
/// `Tdi59CriterionD`/`Tdi59SaturationEntry` fields are plain numeric/boolean
/// descriptors); the classification attached to its component (a),
/// `spectral_pair_focal`, is still printed here for full transparency but is
/// never treated as a TDI-5.9D verdict (Section 4.4, Section 16).
fn print_tdi52_criteria_conditions(report: &Tdi59ExperimentReport) {
    println!();
    println!("=== CONDITIONS PAR CRITÈRE — niveau bloc et agrégat (Section 18) ===");
    for entry in &report.spectral_grid {
        println!();
        println!(
            "TDI-5.9A/C — SKT4 vs SK4 à U_{} : {:#?}",
            entry.horizon, entry.result
        );
    }
    println!();
    println!(
        "TDI-5.9B — valeur marginale du quatrième moment : référence {} ; challenger {}",
        FeatureLayout::Sk.label(),
        FeatureLayout::Sk4.label()
    );
    for entry in &report.fourth_moment_focal {
        println!();
        println!(
            "TDI-5.9B — SK4 vs SK à U_{} : {:#?}",
            entry.horizon, entry.result
        );
    }
    println!();
    println!(
        "TDI-5.9D (a) — réplicat frais de 5.6B (descriptif, non classifié en tant que 5.9D) : \
         référence {} ; challenger {}",
        FeatureLayout::Ck.label(),
        FeatureLayout::Sk.label()
    );
    for entry in &report.spectral_pair_focal {
        println!();
        println!(
            "TDI-5.9D (a) — SK vs CK à U_{} : {:#?}",
            entry.horizon, entry.result
        );
    }
    println!();
    println!("TDI-5.9A (focal) : {:#?}", report.criterion_a);
    println!();
    println!("TDI-5.9B (focal) : {:#?}", report.criterion_b);
    println!();
    println!("TDI-5.9C (loi de décroissance) : {:#?}", report.criterion_c);
    println!();
    println!(
        "TDI-5.9D (saturation, descriptif, aucune classification 4 voies) : {:#?}",
        report.criterion_d
    );
}

/// TDI-5.9 Section 18: the TDI-5.9A and TDI-5.9B focal classifications, the
/// TDI-5.9C decay-law / redundancy-horizon summary, and the TDI-5.9D
/// saturation summary. TDI-5.9A/B/C are preregistered classifications /
/// descriptive summaries; none is forced to a positive result and none is a
/// pass/fail gate. TDI-5.9D is purely descriptive and produces no
/// classification at all (Section 16).
fn print_tdi52_final_verdicts(report: &Tdi59ExperimentReport) {
    println!();
    println!("=== VERDICTS FINAUX (Section 18) ===");

    for (slot, &horizon) in FOCAL_HORIZONS.iter().enumerate() {
        println!(
            "TDI-5.9A — O1/O2 au-delà du spectre exact enrichi (SKT4 vs SK4, U{horizon}) : {}",
    ...
    for (slot, &horizon) in FOCAL_HORIZONS.iter().enumerate() {
        println!(
            "TDI-5.9B — quatrième moment au-delà de {{δ, δ̄, s2, s3}} (SK4 vs SK, U{horizon}) : {}",
    ...
    for index in 0..report.criterion_c.horizons.len() {
        println!(
            "TDI-5.9C — U{} : réduction relative MSE (SKT4 vs SK4) = {:.6}, classification = {}",
    ...
    println!(
        "TDI-5.9C — décroissance monotone (non croissante) : {}",
    ...
    println!(
        "TDI-5.9C — horizon de redondance h★ (première équivalence) : {}",
    ...
    println!(
        "TDI-5.9C — ratios successifs r_(h+1)/r_h : {:?}",
        report.criterion_c.successive_ratios
    );

    for entry in &report.criterion_d.entries {
        println!(
            "TDI-5.9D — U{} — (a) réduction SK-sur-CK = {:.6} IC95%[{:.6}, {:.6}] ; \
             (b) réduction SK4-sur-SK = {:.6} IC95%[{:.6}, {:.6}] ; saturating=(b)<(a)={} ; \
             intervalles chevauchants={}",
            entry.horizon,
            entry.spectral_pair_reduction,
            entry.spectral_pair_interval.lower,
            entry.spectral_pair_interval.upper,
            entry.fourth_moment_reduction,
            entry.fourth_moment_interval.lower,
            entry.fourth_moment_interval.upper,
            if entry.saturating { "oui" } else { "non" },
            if entry.intervals_overlap {
                "oui"
            } else {
                "non"
            },
    ...
    println!(
        "TDI-5.9D — critère purement descriptif (Section 16) : aucune classification \
         succès/échec n'est produite et aucun autre critère n'est subordonné à son résultat."
    );
}

/// Prints the complete TDI-5.9 required raw output (Section 18) for a
/// completed pipeline run. Purely a presentation layer over
/// `Tdi59ExperimentReport`: it has no scale-awareness of its own, so it is
...
fn print_tdi52_required_raw_output(report: &Tdi59ExperimentReport) {
    ...
    for seed_block in FROZEN_BLOCK_ORDER {
        ...
        println!();
        println!(
            "### BLOC {} — normalisations et modèles (Section 18) ###",
            seed_block.label()
        );
    ...
    for entry in &report.spectral_grid {
        print_tdi52_aggregate_comparison(
            &format!("TDI-5.9A/C — SKT4 vs SK4 à U_{}", entry.horizon),
            entry.horizon,
            &entry.comparison,
        );
    }
    ...
    for entry in &report.fourth_moment_focal {
        print_tdi52_aggregate_comparison(
            &format!("TDI-5.9B — SK4 vs SK à U_{}", entry.horizon),
            entry.horizon,
            &entry.comparison,
        );
    }
    ...
    for entry in &report.spectral_pair_focal {
        print_tdi52_aggregate_comparison(
            &format!(
                "TDI-5.9D(a) — SK vs CK à U_{} (descriptif, réplicat frais de 5.6B)",
                entry.horizon
            ),
            entry.horizon,
            &entry.comparison,
...
fn run_termination_smoke() -> Result<(), String> {
    println!("=== TDI-5.9 TERMINATION SMOKE ===");

    // Inherited frozen invariant: the width-6 successor-set space is the
    // exact 2^64. TDI-5.9 generates no width-6 populations, but the
    ...
    // Every generated record now carries exact contraction descriptors and
    // all three exact spectral moments.
    if let Some(first) = report.records.first() {
        ...
        println!(
            "width 3 smoke spectral       : s2={:.6}, s3={:.6}, s4={:.6}",
            first.spectral[0], first.spectral[1], first.spectral[2]
        );
    }
    println!(
    ...
    verify_frozen_ancestor_chain()?;
    println!(
        "frozen ancestor chain verified : {} entries",
        FROZEN_ANCESTOR_CHAIN.len()
    );
    ...
    // Synthetic, bounded records exercising the confirmatory-lineage layouts
    // CK/SK/SKT/SK4/SKT4 without any real generation. Their contraction
    // descriptors and spectral moments (now triples s2/s3/s4) are set by
    // hand.
    ...
    let synthetic_training_width_4 = [
        ...
        Record {
            ...
            spectral: [2.30, 1.90, 1.50],
            overlaps: [0.38; TARGET_HORIZON_COUNT],
            targets_u: [1.70, 1.75, 1.80, 1.85, 1.90, 1.95],
        },
    ];

    // The confirmatory-lineage layouts really do build the extra terms.
    ...
    let sk4_features = feature_layout(&synthetic_training_width_3[0], FeatureLayout::Sk4);
    let skt4_features = feature_layout(&synthetic_training_width_3[0], FeatureLayout::Skt4);
    println!(
        "layout feature widths        : CK={} (attendu {}), SK={} (attendu {}), SKT={} (attendu {}), \
         SK4={} (attendu {}), SKT4={} (attendu {})",
        ck_features.len(),
        CK_FEATURE_COUNT,
        sk_features.len(),
        SK_FEATURE_COUNT,
    ...
    println!(
        ...
        aggregate_fit.block(SeedBlockId::Y).seed_block.label(),
        aggregate_fit.block(SeedBlockId::Z).seed_block.label(),
        aggregate_fit.block(SeedBlockId::Aa).seed_block.label()
    );
    ...
    // Exercise the confirmatory SKT4-vs-SK4 comparison and the four-way
    // classifier (criterion TDI-5.9A) at the primary horizon.
    ...
    println!(
        "identity smoke SKT4 vs SK4 CI : [{:.6}, {:.6}]",
        spectral_focal
            .comparison
            .aggregate_bootstrap
            .standardized_mse
    ...
    println!(
        "identity smoke 5.9A          : classification={}",
        spectral_focal.result.classification.label()
    );

    // Exercise criterion TDI-5.9B (SK4 vs SK, the fourth moment's own
    // marginal value beyond {delta, delta_bar, s2, s3}) at the primary
    // horizon.
    let fourth_moment_focal = evaluate_horizon_comparison(
        primary_horizon_index(),
        &aggregate_fit,
        holdout_refs,
        FeatureLayout::Sk,
    ...
    println!(
        "identity smoke 5.9B          : classification={}",
        fourth_moment_focal.result.classification.label()
    );

    // Exercise criterion TDI-5.9D's component (a) (SK vs CK, descriptive
    // only) at the primary horizon, and the saturation comparison itself.
    let spectral_pair_focal = evaluate_horizon_comparison(
        primary_horizon_index(),
        &aggregate_fit,
        holdout_refs,
        FeatureLayout::Ck,
    ...
    let saturating = fourth_moment_focal.aggregate_relative_reduction
        < spectral_pair_focal.aggregate_relative_reduction;

    println!(
        "identity smoke 5.9D          : SK-sur-CK={:.6}, SK4-sur-SK={:.6}, saturating={saturating}",
        spectral_pair_focal.aggregate_relative_reduction,
        fourth_moment_focal.aggregate_relative_reduction,
    );
    ...
    let pipeline_report =
        run_tdi59_pipeline(&tiny_population_specs).map_err(|error| error.to_string())?;

    println!(
        "identity smoke pipeline      : grille={}, 5.9A[U3]={}, h★={:?}",
    ...
    println!(
        "identity smoke pipeline fit  : block Y model count={}",
        pipeline_report
            .aggregate_fit
            .block(SeedBlockId::Y)
            .models
    ...
    println!(
        "identity smoke pipeline 5.9D : entries={}, first saturating={:?}",
        pipeline_report.criterion_d.entries.len(),
        pipeline_report
            .criterion_d
            .entries
            .first()
            .map(|entry| entry.saturating)
    );
...
/// Name of the environment variable that must carry the exact TDI-5.9
/// full-run confirmation value. See TDI-5.9 preregistration Section 17.
const TDI59_FULL_RUN_CONFIRMATION_VAR: &str = "TDI59_CONFIRM_FULL_RUN";

/// The one accepted value for `TDI59_FULL_RUN_CONFIRMATION_VAR`. Any other
/// value, or the variable being unset, must refuse `--full`.
const TDI59_FULL_RUN_CONFIRMATION_VALUE: &str = "I_ACCEPT_THE_TDI59_FREEZE_RULE";
...
fn tdi59_full_run_confirmed(value: Option<&str>) -> bool {
    value == Some(TDI59_FULL_RUN_CONFIRMATION_VALUE)
}

fn tdi59_usage_error() -> String {
    format!(
        "usage: tdi-independent-overlap-ablation-v59 --termination-smoke|--preflight|--full\n\
         ...
         {TDI59_FULL_RUN_CONFIRMATION_VAR}={TDI59_FULL_RUN_CONFIRMATION_VALUE}"
    )
}
...
fn tdi59_parse_mode(arguments: &[String]) -> Result<Tdi59Mode, String> {
    match arguments {
        [flag] if flag == "--termination-smoke" => Ok(Tdi59Mode::TerminationSmoke),
        [flag] if flag == "--preflight" => Ok(Tdi59Mode::Preflight),
        [flag] if flag == "--full" => Ok(Tdi59Mode::Full),
        _ => Err(tdi59_usage_error()),
    }
}
...
fn main() -> Result<(), String> {
    ...
    match tdi59_parse_mode(&arguments)? {
        Tdi59Mode::TerminationSmoke => run_termination_smoke(),
        Tdi59Mode::Preflight => run_preflight(),
        Tdi59Mode::Full => run_full_experiment(),
    }
}

/// The TDI-5.9 full-run entrypoint. Checks the exact confirmation
/// environment variable *before* any generation, fitting or bootstrap; only
/// when it matches does this verify the full frozen ancestor chain (still
/// before any generation) and then call the real full pipeline exactly once,
/// over the real preregistered `population_specs()`, and print the complete
/// required raw output. See TDI-5.9 preregistration Section 17.
fn run_full_experiment() -> Result<(), String> {
    let confirmation = std::env::var(TDI59_FULL_RUN_CONFIRMATION_VAR).ok();

    if !tdi59_full_run_confirmed(confirmation.as_deref()) {
        return Err(format!(
            "TDI-5.9 full execution requires the exact confirmation environment \
             variable {TDI59_FULL_RUN_CONFIRMATION_VAR}={TDI59_FULL_RUN_CONFIRMATION_VALUE}; \
    ...
    let report = run_tdi59_pipeline(&population_specs())?;
...
/// TDI-5.9 preflight: verifies the complete frozen configuration (the full
/// frozen ancestor chain, seed reservations, population counts, bootstrap
/// constants, pipeline wiring) and prints identities and the exact real-run
/// command, without ever generating a scientific population. See TDI-5.9
/// preregistration Section 17.
fn run_preflight() -> Result<(), String> {
    println!();
    println!("=== TDI-5.9 PREFLIGHT (aucune génération scientifique) ===");

    verify_frozen_ancestor_chain()?;
    println!(
        "chaîne d'ancêtres gelés vérifiée (évaluateur + préenregistrement) : {} entrées",
        FROZEN_ANCESTOR_CHAIN.len()
    );
    ...
    println!(
        ...
        SeedBlockId::Y.label(),
        SeedBlockId::Y.bootstrap_seed(),
        SeedBlockId::Z.label(),
        SeedBlockId::Z.bootstrap_seed(),
        SeedBlockId::Aa.label(),
        SeedBlockId::Aa.bootstrap_seed()
    );
    println!("graine de bootstrap agrégé stratifié            : 0x{AGGREGATE_BOOTSTRAP_SEED:016X}");
    println!(
        "pipeline complet câblé à --full                 : oui (run_tdi59_pipeline, \
         subordonné à {TDI59_FULL_RUN_CONFIRMATION_VAR})"
    );
    ...
    println!("  {TDI59_FULL_RUN_CONFIRMATION_VAR}={TDI59_FULL_RUN_CONFIRMATION_VALUE} \\");
    println!("    bash scripts/reproduce-tdi5.9.sh");
...
mod tests {
    use super::{
        ...
        SK_FEATURE_COUNT, SK_SPECTRAL_FEATURE_COUNT, SK4_FEATURE_COUNT, SKT_FEATURE_COUNT,
        SKT4_FEATURE_COUNT, SPECTRAL_FEATURE_COUNT, SeedBlockId, TARGET_HORIZONS,
        TDI59_FULL_RUN_CONFIRMATION_VALUE, TDI59_FULL_RUN_CONFIRMATION_VAR,
        TOTAL_SEED_RESERVATIONS,
    };
    ...
    fn evaluator_source() -> String {
        read_repo_file("tdi-bench/src/bin/tdi-independent-overlap-ablation-v59.rs")
    }
    ...
    fn record_with_overlap(o1: f64, o2: f64) -> Record {
        Record {
            ...
            spectral: [1.0 + o1, 1.0 + o2, 1.0 + o1 + o2],
    ...
    fn sk_features_add_contraction_then_the_two_spectral_moments() {
        ...
        // SK uses only spectral[0..2] (s2, s3); the third stored moment (s4,
        // here the sentinel 9.9) is deliberately excluded -- SK is inherited
        // unchanged from TDI-5.6.
    ...
    // --- New TDI-5.9 confirmatory layouts SK4/SKT4 (Section 6) ---

    #[test]
    fn sk4_features_add_contraction_then_all_three_spectral_moments() {
        let mut record = record_with_overlap(0.4, 0.6);
        record.contraction = [0.7, 0.3];
        record.spectral = [1.8, 1.4, 1.1];
        let features = super::feature_layout(&record, FeatureLayout::Sk4);

        assert_eq!(features.len(), SK4_FEATURE_COUNT);
        assert_eq!(features.len(), 18);
        assert_eq!(features.len(), FeatureLayout::Sk4.feature_count());
        assert_eq!(&features[..BASELINE_FEATURE_COUNT], &record.baseline);
        let tail = &features[BASELINE_FEATURE_COUNT..];
        assert_eq!(tail, &[0.7, 0.3, 1.8, 1.4, 1.1]);
    }

    #[test]
    fn skt4_features_add_contraction_spectral_then_the_two_overlaps() {
        let (o1, o2) = (0.4, 0.6);
        let mut record = record_with_overlap(o1, o2);
        record.contraction = [0.7, 0.3];
        record.spectral = [1.8, 1.4, 1.1];
        let features = super::feature_layout(&record, FeatureLayout::Skt4);

        assert_eq!(features.len(), SKT4_FEATURE_COUNT);
        assert_eq!(features.len(), 20);
        assert_eq!(features.len(), FeatureLayout::Skt4.feature_count());
        assert_eq!(&features[..BASELINE_FEATURE_COUNT], &record.baseline);
        let tail = &features[BASELINE_FEATURE_COUNT..];
        assert_eq!(tail, &[0.7, 0.3, 1.8, 1.4, 1.1, o1, o2]);
    }

    #[test]
    fn skt4_minus_sk4_is_exactly_the_two_early_overlaps_appended_after_sk4() {
        let record = record_with_overlap(0.33, 0.77);
        let sk4 = super::feature_layout(&record, FeatureLayout::Sk4);
        let skt4 = super::feature_layout(&record, FeatureLayout::Skt4);

        assert_eq!(sk4.len(), SK4_FEATURE_COUNT);
        assert_eq!(skt4.len(), SKT4_FEATURE_COUNT);
        assert_eq!(&skt4[..SK4_FEATURE_COUNT], sk4.as_slice());
        assert_eq!(&skt4[SK4_FEATURE_COUNT..], &[0.33, 0.77]);
    }

    #[test]
    fn confirmatory_layouts_never_perturb_the_baseline_block_and_nest_ck_sk_skt_sk4_skt4() {
        // The 13 baseline features are identical across B0, CK, SK, SKT, SK4
        // and SKT4: only the appended descriptor/overlap block differs, so
        // any SKT-minus-SK or SKT4-minus-SK4 signal is the overlaps' and any
        // SK-minus-CK or SK4-minus-SK signal is spectral. CK is a strict
        // prefix of SK, and of SK4 (SK4 extends SK with s4); SK is a strict
        // prefix of SKT; SK4 is a strict prefix of SKT4.
        let record = record_with_overlap(0.33, 0.77);
        ...
        assert_eq!(&sk4[..BASELINE_FEATURE_COUNT], b0.as_slice());
        assert_eq!(&skt4[..BASELINE_FEATURE_COUNT], b0.as_slice());
        assert_eq!(&sk[..CK_FEATURE_COUNT], ck.as_slice());
        assert_eq!(&skt[..SK_FEATURE_COUNT], sk.as_slice());
        assert_eq!(&sk4[..CK_FEATURE_COUNT], ck.as_slice());
        assert_eq!(&sk4[..SK_FEATURE_COUNT], sk.as_slice());
        assert_eq!(&skt4[..SK4_FEATURE_COUNT], sk4.as_slice());
    }

    #[test]
    fn feature_layout_enumeration_has_ten_entries_including_ck_sk_skt_sk4_skt4() {
        assert_eq!(FeatureLayout::ALL.len(), MODEL_LAYOUT_COUNT);
        assert_eq!(MODEL_LAYOUT_COUNT, 10);
        ...
        assert!(FeatureLayout::ALL.contains(&FeatureLayout::Sk4));
        assert!(FeatureLayout::ALL.contains(&FeatureLayout::Skt4));
        // Linear discriminants are preserved so `layout as usize` indexing is
        // unchanged from TDI-5.2/5.3/5.4/5.5/5.6; SK4/SKT4 extend the
        // sequence at the end (8, 9).
        ...
        assert_eq!(FeatureLayout::Sk4 as usize, 8);
        assert_eq!(FeatureLayout::Skt4 as usize, 9);
    }

    #[test]
    fn confirmatory_layout_counts_match_the_preregistered_values() {
        assert_eq!(CONTRACTION_FEATURE_COUNT, 2);
        assert_eq!(SPECTRAL_FEATURE_COUNT, 3);
        assert_eq!(SK_SPECTRAL_FEATURE_COUNT, 2);
        assert_eq!(CK_FEATURE_COUNT, 15);
        assert_eq!(SK_FEATURE_COUNT, 17);
        assert_eq!(SKT_FEATURE_COUNT, 19);
        assert_eq!(SK4_FEATURE_COUNT, 18);
        assert_eq!(SKT4_FEATURE_COUNT, 20);
    }
    ...
    fn spectral_moments_are_exact_traces_of_kernel_powers() {
        ...
        // fixed point, so trace(P^3) = 4. P^4 = 4 steps around a length-3
        // cycle never returns to the start (4 mod 3 = 1 != 0), so only the
        // fixed point returns: trace(P^4) = 1. Verified independently with
        // exact-fraction arithmetic (Python's `fractions.Fraction`), not just
        // by hand.
        let mut system = TableSystem::new(2).expect("valid width");
        let state = |bits: u64| State::new(bits, 2).expect("valid state");
        system
        ...
        let context = super::AttemptContext::new(2, 0, 0);
        let [s2, s3, s4] = super::spectral_moments(context, &system).expect("moments");

        assert!((s2 - 1.0).abs() < 1e-12, "s2 = {s2}");
        assert!((s3 - 4.0).abs() < 1e-12, "s3 = {s3}");
        assert!((s4 - 1.0).abs() < 1e-12, "s4 = {s4}");
    }

    #[test]
    fn spectral_moments_accumulate_unit_fractions_exactly() {
        // Width-2 kernel mixing branching factors: 0 -> {0, 1} (branch 2),
        // 1 -> {0}, 2 -> {2}, 3 -> {3}. By hand (re-verified independently
        // with exact-fraction arithmetic), trace(P^2) = 3/4 + 1/2 + 1 + 1 =
        // 13/4, trace(P^3) = 5/8 + 1/4 + 1 + 1 = 23/8, and
        // trace(P^4) = 17/16 + 1 + 1 = 49/16, so the exact closed-walk
        // unit-fraction sums must reproduce 3.25, 2.875 and 3.0625.
        let mut system = TableSystem::new(2).expect("valid width");
        let state = |bits: u64| State::new(bits, 2).expect("valid state");
        system
        ...
        assert!((s4 - 3.0625).abs() < 1e-12, "s4 = {s4}");
    }

    #[test]
    fn spectral_moments_are_deterministic() {
        // Same mixed-branching-factor system as
        // `spectral_moments_accumulate_unit_fractions_exactly`: calling
        // `spectral_moments` twice must reproduce every one of s2, s3, s4
        // bit-for-bit (TDI-5.9 preregistration Section 19).
        let mut system = TableSystem::new(2).expect("valid width");
        let state = |bits: u64| State::new(bits, 2).expect("valid state");
        system
            .insert(state(0), Action::Noop, vec![state(0), state(1)])
        ...
        let context = super::AttemptContext::new(2, 0, 0);
        let first = super::spectral_moments(context, &system).expect("moments");
        let second = super::spectral_moments(context, &system).expect("moments");
    ...
    // --- Full-run confirmation guard (Section 17) ---

    #[test]
    fn full_run_confirmation_accepts_only_the_exact_value() {
        assert!(super::tdi59_full_run_confirmed(Some(
            TDI59_FULL_RUN_CONFIRMATION_VALUE
        )));
        assert!(!super::tdi59_full_run_confirmed(None));
        assert!(!super::tdi59_full_run_confirmed(Some("")));
        assert!(!super::tdi59_full_run_confirmed(Some(
            "i_accept_the_tdi59_freeze_rule"
        )));
        // A different experiment's own real, valid token (TDI-6.4's, the
        // most recently built prior experiment) must never unlock TDI-5.9.
        assert!(!super::tdi59_full_run_confirmed(Some(
            "I_ACCEPT_THE_TDI64_FREEZE_RULE"
        )));
        // Nor must its direct scientific ancestor's token (TDI-5.6's), nor a
        // sibling derivation's (TDI-5.8's, an unrelated branch of TDI-5.6).
        assert!(!super::tdi59_full_run_confirmed(Some(
    ...
    fn parse_mode_rejects_a_bare_no_argument_invocation() {
        assert!(super::tdi59_parse_mode(&[]).is_err());
        assert!(super::tdi59_parse_mode(&["--full".to_owned(), "extra".to_owned()]).is_err());
    }

    #[test]
    fn parse_mode_selects_full_only_for_the_exact_single_flag() {
        assert_eq!(
            super::tdi59_parse_mode(&["--full".to_owned()]).unwrap(),
            super::Tdi59Mode::Full
        );
        assert_eq!(
            super::tdi59_parse_mode(&["--preflight".to_owned()]).unwrap(),
            super::Tdi59Mode::Preflight
        );
        assert_eq!(
            super::tdi59_parse_mode(&["--termination-smoke".to_owned()]).unwrap(),
            super::Tdi59Mode::TerminationSmoke
        );
        assert!(super::tdi59_parse_mode(&["--Full".to_owned()]).is_err());
    }

    #[test]
    fn usage_error_mentions_every_flag_and_the_confirmation_variable() {
        let usage = super::tdi59_usage_error();
        assert!(usage.contains("--termination-smoke"));
        assert!(usage.contains("--preflight"));
        assert!(usage.contains("--full"));
        assert!(usage.contains(TDI59_FULL_RUN_CONFIRMATION_VAR));
        assert!(usage.contains(TDI59_FULL_RUN_CONFIRMATION_VALUE));
    }
    ...
    fn full_run_refuses_before_any_work_without_the_confirmation_token() {
        ...
        if std::env::var(TDI59_FULL_RUN_CONFIRMATION_VAR).is_ok() {
    ...
    fn run_full_experiment_is_wired_to_the_real_pipeline_on_the_accepted_path() {
        ...
        assert!(
            body.contains("run_tdi59_pipeline(&population_specs())"),
            "accepted path must call the real pipeline over the real specs"
        );
        assert!(body.contains("tdi59_full_run_confirmed"));
        assert!(body.contains("verify_frozen_ancestor_chain"));
        assert!(body.contains("print_tdi52_required_raw_output"));
    }

    #[test]
    fn preflight_is_wired_to_verify_the_frozen_ancestor_chain_and_seed_reservations() {
        let source = evaluator_source();
        let start = source
        ...
        assert!(body.contains("verify_frozen_ancestor_chain"));
        assert!(body.contains("validate_preregistered_seed_reservations"));
    }
    ...
    fn termination_smoke_uses_only_bounded_specs_never_the_real_ones() {
        ...
        let end = source[start..]
            .find("\nfn tdi59_full_run_confirmed")
            .map(|offset| start + offset)
            .expect("tdi59_full_run_confirmed must follow run_termination_smoke");
        let body = &source[start..end];
    ...
    #[test]
    fn seed_blocks_are_y_z_aa_and_disjoint_from_every_prior_block() {
        let ids: Vec<_> = SEED_BLOCKS.iter().map(|b| b.id).collect();
        assert_eq!(ids, vec![SeedBlockId::Y, SeedBlockId::Z, SeedBlockId::Aa]);

        // Fresh offsets (10.000e9..10.230e9), structurally above every prior
        // block: the TDI-5.9 preregistration Section 9 documents TDI-6.4 (the
        // most recently allocated block) as consuming seeds only up to
        // ~9.23e9, while TDI-5.9 starts at 1.0e10.
        for block in SEED_BLOCKS {
            for seed in [
                block.training_width_3_seed,
                block.holdout_width_3_seed,
                ...
                assert!(seed >= 10_000_000_000);
            }
        }

        // Within each block, the four populations are spaced 10,000,000 apart
        // in `PopulationKind::ALL` order.
        for block in SEED_BLOCKS {
            let seeds = [
                block.training_width_3_seed,
                block.holdout_width_3_seed,
        ...
        // The three blocks are pairwise disjoint, each spaced 100,000,000
        // apart -- far beyond any population's generation budget (at most a
        // few million seeds, Section 9), so the reservations cannot collide.
        assert_eq!(
            SEED_BLOCKS[1].training_width_3_seed - SEED_BLOCKS[0].training_width_3_seed,
            100_000_000
        );
        assert_eq!(
            SEED_BLOCKS[2].training_width_3_seed - SEED_BLOCKS[1].training_width_3_seed,
            100_000_000
        );

        // Bootstrap seeds are the fresh, distinct Y/Z/AA constants.
        let boots: Vec<_> = SEED_BLOCKS.iter().map(|b| b.bootstrap_seed).collect();
        assert_eq!(
            boots,
            vec![
                0x5444_4935_3900_0001_u64,
                0x5444_4935_3900_0002,
                0x5444_4935_3900_0003
            ]
        );
        assert_eq!(AGGREGATE_BOOTSTRAP_SEED, 0x5444_4935_3900_4700);
    ...
    #[test]
    fn skt4_ridge_fit_and_prediction_are_deterministic_and_reconstruct_overlap() {
        let records: Vec<Record> = (0..24)
            .map(|i| {
                let o1 = 0.10 + 0.02 * f64::from(i % 7);
                let o2 = 0.20 + 0.015 * f64::from(i % 5);
        ...
        let targets = super::overlap_values(&records, super::primary_horizon_index());
        let design = super::feature_matrix(&records, |record| {
            super::feature_layout(record, FeatureLayout::Skt4)
        });

        let first = super::fit_ridge(&design, &targets).expect("ridge fit");
        let second = super::fit_ridge(&design, &targets).expect("ridge fit");
        assert_eq!(first.coefficients, second.coefficients);
        // Per-feature scalers cover all 20 SKT4 features; coefficients carry
        // an additional intercept at index 0.
        assert_eq!(first.means.len(), SKT4_FEATURE_COUNT);
        assert_eq!(first.coefficients.len(), SKT4_FEATURE_COUNT + 1);
    ...
    // --- Criterion TDI-5.9D descriptor saturation (Section 16) ---

    #[test]
    fn confidence_intervals_overlap_detects_overlap_and_disjointness() {
        let a = super::ConfidenceInterval {
            lower: 0.01,
            median: 0.02,
            upper: 0.03,
        };
        let b = super::ConfidenceInterval {
            lower: 0.02,
            median: 0.025,
            upper: 0.04,
        };
        ...
        let c = super::ConfidenceInterval {
            lower: 0.05,
            median: 0.06,
            upper: 0.07,
        };
        assert!(!super::confidence_intervals_overlap(a, c));
        assert!(!super::confidence_intervals_overlap(c, a));

        // Touching endpoints count as overlapping (closed intervals).
        let d = super::ConfidenceInterval {
            lower: 0.03,
            median: 0.035,
            upper: 0.05,
        };
        assert!(super::confidence_intervals_overlap(a, d));
    }

    #[test]
    fn saturation_entry_saturating_flag_and_overlap_are_computed_correctly_from_hand_built_reductions()
     {
        // (a) SK-over-CK reduction 0.05 with CI [0.03, 0.07]; (b) SK4-over-SK
        // reduction 0.01 with CI [0.00, 0.02] -- disjoint from (a). (b) < (a),
        // so saturating = true, and the two intervals do not overlap.
        let spectral_pair_interval = super::ConfidenceInterval {
            lower: 0.03,
            median: 0.05,
            upper: 0.07,
        };
        let fourth_moment_interval = super::ConfidenceInterval {
            lower: 0.00,
            median: 0.01,
            upper: 0.02,
        };
        let entry = super::Tdi59SaturationEntry {
            horizon: 3,
            spectral_pair_reduction: 0.05,
            spectral_pair_interval,
            fourth_moment_reduction: 0.01,
            fourth_moment_interval,
            saturating: 0.01 < 0.05,
            intervals_overlap: super::confidence_intervals_overlap(
                spectral_pair_interval,
                fourth_moment_interval,
            ),
        };
        assert!(entry.saturating);
        assert!(!entry.intervals_overlap);

        // Flip: (b) > (a) with overlapping intervals => saturating = false,
        // intervals_overlap = true.
        let spectral_pair_interval_2 = super::ConfidenceInterval {
            lower: 0.00,
            median: 0.01,
            upper: 0.03,
        };
        let fourth_moment_interval_2 = super::ConfidenceInterval {
            lower: 0.02,
            median: 0.04,
            upper: 0.06,
        };
        let saturating_2 = 0.04 < 0.01;
        let overlap_2 =
            super::confidence_intervals_overlap(spectral_pair_interval_2, fourth_moment_interval_2);
        assert!(!saturating_2);
        assert!(overlap_2);
    }

    #[test]
    fn criterion_d_entry_carries_no_pass_fail_classification_field() {
        // TDI-5.9D is purely descriptive (Section 16): `Tdi59SaturationEntry`
        // must never carry a `CriterionCClassification` field, so its Debug
        // output can never mention one -- unlike `Tdi59CriterionA`, which
        // does classify and whose Debug output does mention one.
        let interval = super::ConfidenceInterval {
            lower: 0.0,
            median: 0.0,
            upper: 0.0,
        };
        let entry = super::Tdi59SaturationEntry {
            horizon: 3,
            spectral_pair_reduction: 0.0,
            spectral_pair_interval: interval,
            fourth_moment_reduction: 0.0,
            fourth_moment_interval: interval,
            saturating: false,
            intervals_overlap: false,
        };
        let entry_debug = format!("{entry:?}");
        assert!(
            !entry_debug.contains("Classification"),
            "TDI-5.9D must carry no pass/fail classification: {entry_debug}"
        );

        let criterion_a = super::Tdi59CriterionA {
            focal_classifications: [CriterionCClassification::Equivalent; 2],
        };
        let criterion_a_debug = format!("{criterion_a:?}");
        assert!(criterion_a_debug.contains("Equivalent"));
    }

    // --- Tiny end-to-end pipeline wiring for all four TDI-5.9 criteria ---

    #[test]
    fn tiny_pipeline_wires_all_four_tdi59_criteria() {
        // A `target_count` of 1 per population (never anything larger, e.g.
        // not 60 -- an oversized tiny-pipeline `target_count` previously made
        // a single TDI-6.4 test consume ~98% of the whole suite's wall-clock
        // time). Mirrors the identical technique already used by
        // `run_termination_smoke`'s own tiny pipeline invocation.
        let tiny_specs = super::population_specs().map(|spec| super::PopulationSpec {
            target_count: 1,
            ..spec
        });

        let report = super::run_tdi59_pipeline(&tiny_specs).expect("tiny pipeline must succeed");

        assert_eq!(report.spectral_grid.len(), TARGET_HORIZONS.len());
        assert_eq!(
            report.criterion_a.focal_classifications.len(),
            FOCAL_HORIZONS.len()
        );
        assert_eq!(
        ...
        assert_eq!(report.criterion_c.horizons.len(), TARGET_HORIZONS.len());
        assert_eq!(
            report.criterion_c.relative_reductions.len(),
            TARGET_HORIZONS.len()
        );
        assert_eq!(
            report.criterion_c.classifications.len(),
            TARGET_HORIZONS.len()
        );
        assert_eq!(
            report.criterion_c.successive_ratios.len(),
            TARGET_HORIZONS.len() - 1
        );
        assert_eq!(report.fourth_moment_focal.len(), FOCAL_HORIZONS.len());
        assert_eq!(report.spectral_pair_focal.len(), FOCAL_HORIZONS.len());
        assert_eq!(report.criterion_d.entries.len(), FOCAL_HORIZONS.len());

        // TDI-5.9D's entries are at exactly the focal horizons, in order, and
        // their `saturating` / `intervals_overlap` flags are consistent with
        // the two reported reductions and intervals (the same pure logic
        // exercised by hand above, now over data the pipeline itself
        // produced).
        let horizons: Vec<usize> = report
            .criterion_d
            .entries
            .iter()
            .map(|entry| entry.horizon)
            .collect();
        assert_eq!(horizons, FOCAL_HORIZONS.to_vec());

        for entry in &report.criterion_d.entries {
            assert!(entry.spectral_pair_reduction.is_finite());
            assert!(entry.fourth_moment_reduction.is_finite());
            assert_eq!(
                entry.saturating,
                entry.fourth_moment_reduction < entry.spectral_pair_reduction
            );
            assert_eq!(
                entry.intervals_overlap,
                super::confidence_intervals_overlap(
                    entry.spectral_pair_interval,
                    entry.fourth_moment_interval
                )
            );
        }
    }

    // --- Frozen ancestors must never change under TDI-5.9 (Section 23) ---

    #[test]
    fn frozen_ancestor_hashes_are_unchanged() {
        for entry in super::FROZEN_ANCESTOR_CHAIN {
            super::verify_ancestor_entry(entry)
                .unwrap_or_else(|error| panic!("frozen ancestor changed: {error}"));
        }
        assert_eq!(super::FROZEN_ANCESTOR_CHAIN.len(), 13);
    }

    #[test]
    fn frozen_ancestor_chain_covers_the_full_thirteen_entry_build_order() {
        let labels: Vec<&str> = super::FROZEN_ANCESTOR_CHAIN
            .iter()
            .map(|&(label, ..)| label)
            .collect();

        // Chronological build order (preregistration Section 1, Section 3):
        // TDI-6.5 was designed and merged before TDI-6.4 despite the
        // numbering.
        assert_eq!(
            labels,
            vec![
                "TDI-5.1", "TDI-5.2", "TDI-5.3", "TDI-5.4", "TDI-5.5", "TDI-5.6", "TDI-5.7",
                "TDI-5.8", "TDI-6.1", "TDI-6.2", "TDI-6.3", "TDI-6.5", "TDI-6.4",
            ]
        );
    }

    #[test]
    fn verify_frozen_ancestor_chain_passes_over_the_real_committed_files() {
        super::verify_frozen_ancestor_chain()
            .expect("the real frozen ancestor chain must verify against the files on disk");
    }

    #[test]
    fn verify_ancestor_entry_fails_loudly_on_a_wrong_evaluator_hash() {
        let tampered = (
            "TDI-5.6",
            "tdi-bench/src/bin/tdi-independent-overlap-ablation-v56.rs",
        ...
        let error =
            super::verify_ancestor_entry(tampered).expect_err("a wrong hash must fail loudly");
        assert!(error.contains("frozen ancestor evaluator changed"));
        assert!(error.contains("TDI-5.6"));
    }

    #[test]
    fn verify_ancestor_entry_fails_loudly_on_a_wrong_preregistration_hash() {
        let tampered = (
            "TDI-5.6",
            "tdi-bench/src/bin/tdi-independent-overlap-ablation-v56.rs",
            "0820274b3edb58a6e123c612dbed8dd8a1725221240365f142d9510404e1d1b2",
        ...
        let error =
            super::verify_ancestor_entry(tampered).expect_err("a wrong hash must fail loudly");
        assert!(error.contains("frozen ancestor preregistration changed"));
        assert!(error.contains("TDI-5.6"));
    }

    #[test]
    fn verify_ancestor_entry_passes_on_every_real_chain_entry() {
        for entry in super::FROZEN_ANCESTOR_CHAIN {
            assert!(super::verify_ancestor_entry(entry).is_ok());
        }
    }
}
