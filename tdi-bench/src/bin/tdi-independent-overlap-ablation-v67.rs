...
#[derive(Clone, Debug)]
struct Tdi67ExperimentReport {
    families: Vec<FamilyReport>,
    criterion_a: Tdi67CriterionA,
    criterion_b: Tdi67CriterionB,
    criterion_c: Tdi67CriterionC,
    criterion_d: Tdi67CriterionD,
}
...
fn run_tdi67_pipeline(
    population_specs: &[PopulationSpec],
) -> Result<Tdi67ExperimentReport, String> {
    ...
    let criterion_b = Tdi67CriterionB {
        repaired,
        non_repairs,
    };
    ...
    let criterion_d = Tdi67CriterionD {
...
fn print_tdi67_criteria_conditions(report: &Tdi67ExperimentReport) {
    println!();
...
fn print_tdi67_final_verdicts(report: &Tdi67ExperimentReport) {
    println!();
...
fn print_tdi67_required_raw_output(report: &Tdi67ExperimentReport) {
    ...
    print_tdi67_criteria_conditions(report);
    print_tdi67_final_verdicts(report);
}
...
fn run_termination_smoke() -> Result<(), String> {
    ...
    let pipeline_report =
        run_tdi67_pipeline(&tiny_population_specs).map_err(|error| error.to_string())?;
    ...
    print_tdi67_required_raw_output(&pipeline_report);
...
const TDI65_FULL_RUN_CONFIRMATION_VAR: &str = "TDI67_CONFIRM_FULL_RUN";

/// The one accepted value for `TDI65_FULL_RUN_CONFIRMATION_VAR`. Any other
/// value, or the variable being unset, must refuse `--full`.
const TDI65_FULL_RUN_CONFIRMATION_VALUE: &str = "I_ACCEPT_THE_TDI67_FREEZE_RULE";
...
fn tdi67_full_run_confirmed(value: Option<&str>) -> bool {
    value == Some(TDI65_FULL_RUN_CONFIRMATION_VALUE)
}

fn tdi67_usage_error() -> String {
    format!(
        "usage: tdi-independent-overlap-ablation-v67 --termination-smoke|--preflight|--full\n\
...
fn tdi67_parse_mode(arguments: &[String]) -> Result<Tdi67Mode, String> {
    match arguments {
        [flag] if flag == "--termination-smoke" => Ok(Tdi67Mode::TerminationSmoke),
        [flag] if flag == "--preflight" => Ok(Tdi67Mode::Preflight),
        [flag] if flag == "--full" => Ok(Tdi67Mode::Full),
        _ => Err(tdi67_usage_error()),
    }
}
...
fn main() -> Result<(), String> {
    ...
    match tdi67_parse_mode(&arguments)? {
        Tdi67Mode::TerminationSmoke => run_termination_smoke(),
        Tdi67Mode::Preflight => run_preflight(),
        Tdi67Mode::Full => run_full_experiment(),
    }
}
...
fn run_full_experiment() -> Result<(), String> {
    ...
    let report = run_tdi67_pipeline(&population_specs())?;

    print_tdi67_required_raw_output(&report);
...
fn run_preflight() -> Result<(), String> {
    ...
    println!(
        "pipeline complet câblé à --full                 : oui (run_tdi67_pipeline, \
...
mod tests {
    ...
    fn evaluator_source() -> String {
        read_repo_file("tdi-bench/src/bin/tdi-independent-overlap-ablation-v67.rs")
    }
    ...
    fn full_run_confirmation_accepts_only_the_exact_value() {
        assert!(super::tdi67_full_run_confirmed(Some(
            TDI65_FULL_RUN_CONFIRMATION_VALUE
        )));
        assert!(!super::tdi67_full_run_confirmed(None));
        assert!(!super::tdi67_full_run_confirmed(Some("")));
        assert!(!super::tdi67_full_run_confirmed(Some(
    ...
    fn parse_mode_rejects_a_bare_no_argument_invocation() {
        assert!(super::tdi67_parse_mode(&[]).is_err());
        assert!(super::tdi67_parse_mode(&["--full".to_owned(), "extra".to_owned()]).is_err());
    }

    #[test]
    fn parse_mode_selects_full_only_for_the_exact_single_flag() {
        assert_eq!(
            super::tdi67_parse_mode(&["--full".to_owned()]).unwrap(),
            super::Tdi67Mode::Full
        );
        assert_eq!(
            super::tdi67_parse_mode(&["--preflight".to_owned()]).unwrap(),
            super::Tdi67Mode::Preflight
        );
        assert_eq!(
            super::tdi67_parse_mode(&["--termination-smoke".to_owned()]).unwrap(),
            super::Tdi67Mode::TerminationSmoke
        );
        assert!(super::tdi67_parse_mode(&["--Full".to_owned()]).is_err());
    }

    #[test]
    fn usage_error_mentions_every_flag_and_the_confirmation_variable() {
        let usage = super::tdi67_usage_error();
    ...
    fn run_full_experiment_is_wired_to_the_real_pipeline_on_the_accepted_path() {
        ...
        assert!(
            body.contains("run_tdi67_pipeline(&population_specs())"),
            "accepted path must call the real pipeline over the real specs"
        );
        assert!(body.contains("tdi67_full_run_confirmed"));
        assert!(body.contains("print_tdi67_required_raw_output"));
    }
    ...
    fn termination_smoke_uses_only_bounded_specs_never_the_real_ones() {
        ...
        let end = source[start..]
            .find("\nfn tdi67_full_run_confirmed")
            .map(|offset| start + offset)
            .expect("tdi67_full_run_confirmed must follow run_termination_smoke");
        let body = &source[start..end];
