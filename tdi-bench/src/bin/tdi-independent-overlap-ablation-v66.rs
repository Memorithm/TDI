...
//! `tdi66_full_run_confirmed`). No commit, test or CI run supplies that
//! token.
...
impl SeedBlockId {
    ...
    fn population_base_seed(self) -> u64 {
        6_200_000_000 + self.family.index() * 300_000_000 + u64::from(self.block) * 100_000_000
    }

    /// `0x5444_4936_3600_0000 + (SEED_BLOCK_COUNT·f + b) + 1` (Section 9) — the
    /// series ASCII scheme "TDI" + "6" + "6".
    fn bootstrap_seed(self) -> u64 {
        0x5444_4936_3600_0000
...
/// TDI-6.6's single changed factor (preregistration Section 4.1): whose
/// statistics standardize the frozen coefficients at transfer time.
///
/// The coefficients `β` are **never refitted** in any arm (Section 4.5); only
/// the standardization changes. Representing the choice as an enum rather than
/// a pair of booleans makes the combinations outside Section 4.1's table
/// unrepresentable.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum TransferArm {
    /// **A0** — source-domain feature statistics and source target scaler: the
    /// TDI-6.5C behaviour, carried over unchanged so that the A1-vs-A0
    /// comparison is paired on identical populations.
    SourceStandardized,
    /// **A1** — target-domain feature statistics, source target scaler.
    ///
    /// This is the experiment. Computing a feature mean and standard deviation
    /// in the target domain requires target *inputs* only, so A1 is ordinary
    /// unsupervised domain adaptation and is a deployable procedure.
    FeatureRestandardized,
    /// **A2** — target-domain feature statistics *and* target scaler.
    ///
    /// Fitting the target scaler reads the target domain's `U_h` values — the
    /// very quantity being predicted — so **A2 is not a prediction method**. It
    /// is an upper bound that localizes whether a residual failure lives in the
    /// feature scale or the target scale, and every output line naming it is
    /// labelled `oracle` (Sections 14 and 19).
    OracleRestandardized,
}

impl TransferArm {
    const ALL: [Self; 3] = [
        Self::SourceStandardized,
        Self::FeatureRestandardized,
        Self::OracleRestandardized,
    ];

    fn label(self) -> &'static str {
        match self {
            Self::SourceStandardized => "A0-source",
            Self::FeatureRestandardized => "A1-feature-restandardized",
            Self::OracleRestandardized => "A2-oracle-restandardized",
        }
    }

    /// Whether this arm's construction reads any target-domain label.
    ///
    /// True for `A2` alone. Every reporting path consults this rather than
    /// re-deriving the property, so the `oracle` labelling cannot disagree with
    /// what the arm actually does.
    const fn uses_target_labels(self) -> bool {
        matches!(self, Self::OracleRestandardized)
    }
...
///
/// `target_training_records` are the target family's combined **training**
/// populations, per seed block, in frozen block order — never the holdout being
/// scored (Section 4.2), so the design is not transductive.
///
/// The coefficients are cloned verbatim in every arm. `A0` returns the source
/// fit unchanged; `A1` replaces each model's feature means and scales with the
/// target domain's; `A2` additionally refits the target scalers, which is the
/// step that reads target labels.
fn restandardize_aggregate_fit(
        ...
        .zip(target_training_records)
    {
        if records.is_empty() {
            return Err(format!(
                "re-standardization for block {} received no target training records",
                seed_block.label()
            ));
        }
        ...
        // One statistics computation per layout: features do not depend on the
        // horizon, so recomputing per horizon would repeat identical work and
        // invite a horizon/layout indexing mistake.
        let mut layout_statistics = Vec::with_capacity(MODEL_LAYOUT_COUNT);

        for layout in FeatureLayout::ALL {
            let matrix = feature_matrix(records, |record| model_features(record, layout));

            layout_statistics.push(feature_standardization(&matrix)?);
        }
        ...
        for horizon_index in 0..TARGET_HORIZON_COUNT {
            for layout in FeatureLayout::ALL {
                ...
                let (means, scales) = &layout_statistics[layout as usize];

                if source_model.means.len() != means.len() {
                    return Err(format!(
                        "layout {layout:?} feature count changed between fit ({}) and \
                         re-standardization ({})",
                        source_model.means.len(),
                        means.len()
                    ));
                }
                ...
                models.push(RidgeModel {
                    means: means.clone(),
                    scales: scales.clone(),
                    coefficients: source_model.coefficients.clone(),
                });
            }
        }

        // A1 keeps the source target scaler: replacing it would read the target
        // domain's `U_h` values and is what makes A2 an oracle.
    ...
    let blocks: [BlockModelFit; SEED_BLOCK_COUNT] =
        blocks.try_into().map_err(|values: Vec<BlockModelFit>| {
            format!(
                "expected {SEED_BLOCK_COUNT} re-standardized blocks, received {}",
                values.len()
            )
        })?;

    // Re-enters the same frozen-block-order validation the original fit passed,
    // so a re-standardized fit cannot carry a block order the evaluator would
    // reject.
...
fn run_preflight() -> Result<(), String> {
    ...
    println!("  {TDI65_FULL_RUN_CONFIRMATION_VAR}={TDI65_FULL_RUN_CONFIRMATION_VALUE} \\");
...
mod tests {
    use super::{
        ...
        TDI65_FULL_RUN_CONFIRMATION_VALUE, TDI65_FULL_RUN_CONFIRMATION_VAR,
    ...
    fn usage_error_mentions_every_flag_and_the_confirmation_variable() {
        ...
        assert!(usage.contains(TDI65_FULL_RUN_CONFIRMATION_VAR));
        assert!(usage.contains(TDI65_FULL_RUN_CONFIRMATION_VALUE));
    }
    ...
    fn full_run_refuses_before_any_work_without_the_confirmation_token() {
        ...
        if std::env::var(TDI65_FULL_RUN_CONFIRMATION_VAR).is_ok() {
