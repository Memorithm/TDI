//! TDI-6.5 generator-family robustness of the literal-spectral control (does the
//! {O1,O2}-beyond-{contraction + exact spectral + literal spectral} signal
//! survive `g = 1 - |λ2|` and `τ_ε` *across a family of exact generators*?).
//!
//! This file derives from the frozen TDI-6.1 evaluator
//! (`tdi-independent-overlap-ablation-v61.rs`) by changing exactly one factor —
//! the **generator**: TDI-6.1's single base generator is replaced by the frozen
//! TDI-5.7 family of four structurally distinct exact generators F0–F3. It
//! therefore grafts TDI-6.1's two *literal* non-exact spectral descriptors — the
//! spectral gap `g = 1 - |λ2|` and the ε-mixing time `τ_ε` of the one-step Noop
//! kernel — onto TDI-5.7's exact mask-based multi-family machinery. TDI-5.1 …
//! TDI-5.7, TDI-6.1 and TDI-6.2 remain frozen and untouched.
//!
//! Because the baseline against which the overlaps are tested is now the
//! literal-spectral **GK** layout (SK + g + τ_ε), not the exact **SK** layout of
//! TDI-5.7, the confirmatory comparison becomes **GKT vs GK evaluated
//! independently within each family** — the TDI-6.1A comparison replicated
//! across the TDI-5.7 family, in place of TDI-5.7's exact SKT-vs-SK. It combines
//! two frozen designs without any new scientific mechanism:
//!
//!   * inherited unchanged from TDI-6.1: the one-step Noop kernel `P`, the two
//!     non-exact spectral descriptors `g`/`τ_ε` and their three-method
//!     cross-validation, the CK/SK/GK/GKT layouts, and the localized non-exact
//!     determinism discipline (only `g`, `τ_ε` are non-exact f64 quantities);
//!   * inherited unchanged from TDI-5.7: the four deterministic successor-mask
//!     generation rules `F0Base`/`F1Sparse`/`F2Dense`/`F3Local`, each assembled
//!     by the unchanged frozen `build_system`; the per-family three-block
//!     population structure; the F0→F1 transfer pair; and the replication /
//!     heterogeneity / transfer / descriptor-drift criteria template;
//!   * new only in the *combination*: 48 fresh, pairwise-disjoint seed
//!     reservations (4 families x 3 seed blocks x 4 populations = 480,000
//!     accepted records, 4 × 3 × 40,000), disjoint from every prior block, with
//!     fresh `TDI6`/`35` bootstrap seeds; criteria 6.5A replication (GKT-vs-GK
//!     Beneficial at U3 and U6 for all four families), 6.5B across-family
//!     effect-size heterogeneity, 6.5C F0→F1 transfer, and 6.5D per-family
//!     descriptor drift over the six descriptors δ, δ̄, s2, s3, g, τ_ε plus the
//!     per-family GK-vs-SK diagnostic; per-family per-horizon GKT-vs-GK
...
//! `tdi65_full_run_confirmed`). No commit, test or CI run supplies that
//! token.
